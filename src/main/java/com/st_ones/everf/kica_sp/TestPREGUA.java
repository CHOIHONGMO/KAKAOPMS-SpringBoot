...
public class TestPREGUA extends HttpServlet {
    ...
    public void setProc() {
		...
		System.out.println( "----------------------------------------------------------------------------" );
		System.out.println("======= PREGUA SGIxLinker 송신 전");

		SGIxLinker xLinker = new SGIxLinker("D:/DMMRO/workspace/mro/wiseconfig/KICA_SGIxLinker/conf/sendinfo1.properties", "send_jsp", true);
		System.out.println("=========PREGUA SGIxLinker 송신 후");
					
		boolean isOK = xLinker.doSendProcess(xmlDoc, null);
		System.out.println("PREGUA isOK========" + isOK);
		if (!isOK) {
			System.out.println("PREGUA SGIxLinker 에러메시지:" + xLinker.getErrorMsg());
			System.out.println("PREGUA SGIxLinker 에러메시지:" + xLinker.getErrorMsg());
			return;
		}
		
		String recvXml = xLinker.getRecvXmlData();
		System.out.println("PREGUA recvXml----------------------" + recvXml);
	}
	...
	public static String composeXML(String templatePath, String docCode)
	{
		...
		/* 보증서(증권) */
		String bond_numb_text ="098745632109876543210987654321";					/*	[선택]	  보증서(증권)번호           */
		/* 보증내용 */
...
//				FileWriteUtil.genFileCreate( "D:/DMMRO/workspace/mro/servlet_src/kica_sp/samplePREGUA.xml", xmlDoc);
				FileWriteUtil.genFileCreate( "D:/app/mro/servlet_src/kica_sp/samplePREGUA.xml", xmlDoc);
			}else{
				System.out.println(dataToXml.getErrorMsg());
			}
		}catch(Exception _e){
