package com.st_ones.batch.delyAlarmIf.web;

import com.st_ones.batch.EverJob;
import com.st_ones.batch.batchLogCom.service.BatchLogCommon_Service;
import com.st_ones.batch.delyAlarmIf.service.DelyAlarmIfService;
 ...
 */
@Service
public class DelyAlarmIf extends EverJob {
	
	@Autowired private BatchLogCommon_Service batchLogCommon_Service;
    @Autowired private DelyAlarmIfService delyAlarmIfService;
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        ...
        if (super.isAllowedToRunBatch(context)) {
            ...
            delyAlarmIfService = SpringContextUtil.getBean(DelyAlarmIfService.class);
            
            String msg = "";
            int totalCount = 0;
            try {
                // 납품예정 알림
                msg = delyAlarmIfService.doAlarmInvoiceInfo(new HashMap<String, String>());
                jobRlt = "S";
            } catch (Exception e) {
                ...
                try {
                    ...
                    logData.put("JOB_ID", "DelyAlarmIf");
                    logData.put("JOB_NM", "납품예정 알림(운영사 품목담당자 => 공급사 납품담당자), 매일 09시, 희망납기일 D-1 시점에서 1회 발송");
                    logData.put("JOB_KEY", "");
