...
<e:ui locale="${ses.countryCd}" lang="${ses.langCd}" dateFmt="${ses.dateFormat}">
    ...
    function doSearch() {
        ...
        store.setGrid([grid]);
        store.load(baseUrl + 'my03092_doSearch', function() {
        	if(grid.getRowCount() == 0){
				...
				grid.setColMerge(['RELAT_YN','CUST_NM']);

                grid.setProperty("footerVisible", val);
                grid.setRowFooter("MAKER_PART_NO", footerTxt);
	...
	*/
    ...
    <e:window id="MY03_092" onReady="init" initData="${initData}" title="${fullScreenName}" breadCrumbs="${breadCrumb }">
...
<%--				<e:label for="DATE_SEL" title="${MY03_092_001}" />--%>
<%--				<e:field>--%>
...
<%--						<e:radio id="DATE_SEL_D" name="DATE_SEL_D" value="D" label="${MY03_092_002}" disabled="false" readOnly="false" />--%>
<%--						<e:radio id="DATE_SEL_W" name="DATE_SEL_W" value="W" label="${MY03_092_003}" disabled="false" readOnly="false" />--%>
<%--						<e:radio id="DATE_SEL_B" name="DATE_SEL_B" value="B" label="${MY03_092_004}" disabled="false" readOnly="false" />--%>
<%--						<e:radio id="DATE_SEL_M" name="DATE_SEL_M" value="M" label="${MY03_092_005}" disabled="false" readOnly="false" checked="true" />--%>
<%--&lt;%&ndash;						<e:radio id="DATE_SEL_S" name="DATE_SEL_S" value="S" label="${MY03_092_006}" disabled="false" readOnly="false" />&ndash;%&gt;--%>
<%--					</e:radioGroup>--%>
<%--				</e:field>--%>
			</e:row>
            <e:row>
				...
				<e:field>
					...
					<%--
					<e:select id="YEAR_FROM" name="YEAR_FROM" value="${thisYear}" options="${yearFromOptions}" width="${form_YEAR_FROM_W}" disabled="${form_YEAR_FROM_D}" readOnly="${form_YEAR_FROM_RO}" required="${form_YEAR_FROM_R}" placeHolder="" />
					<e:select id="MONTH_FROM" name="MONTH_FROM" value="${thisMonth}" options="${monthFromOptions}" width="${form_MONTH_FROM_W}" disabled="${form_MONTH_FROM_D}" readOnly="${form_MONTH_FROM_RO}" required="${form_MONTH_FROM_R}" placeHolder="" />
					<e:text>&nbsp;~&nbsp;</e:text>
					<e:select id="YEAR_TO" name="YEAR_TO" value="${thisYear}" options="${yearToOptions}" width="${form_YEAR_TO_W}" disabled="${form_YEAR_TO_D}" readOnly="${form_YEAR_TO_RO}" required="${form_YEAR_TO_R}" placeHolder="" />
					<e:select id="MONTH_TO" name="MONTH_TO" value="${thisMonth}" options="${monthToOptions}" width="${form_MONTH_TO_W}" disabled="${form_MONTH_TO_D}" readOnly="${form_MONTH_TO_RO}" required="${form_MONTH_TO_R}" placeHolder="" />
