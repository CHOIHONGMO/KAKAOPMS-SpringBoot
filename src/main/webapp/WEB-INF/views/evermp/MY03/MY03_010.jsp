...
<e:ui locale="${ses.countryCd}" lang="${ses.langCd}" dateFmt="${ses.dateFormat}">
    ...
    <script>
	    ...
	    function doSearch() {
	        ...
	        store.setGrid([grid]);
	        store.load(baseUrl + 'my03010_doSearch', function() {
    ...
    <e:window id="MY03_010" onReady="init" initData="${initData}" title="${fullScreenName}" breadCrumbs="${breadCrumb }">
		<e:searchPanel id="form" title="${msg.M9999}" labelWidth="${labelWidth}" width="100%" columnCount="3" onEnter="doSearch" useTitleBar="false">
            <e:row>
				<e:label for="CMS_MAPPING_START_DATE" title="${form_CMS_MAPPING_START_DATE_N}"/>
				<e:field>
					<e:inputDate id="CMS_MAPPING_START_DATE" name="CMS_MAPPING_START_DATE" value="${fromDate }" width="${inputDateWidth}" datePicker="true" required="${form_CMS_MAPPING_START_DATE_R}" disabled="${form_CMS_MAPPING_START_DATE_D}" readOnly="${form_CMS_MAPPING_START_DATE_RO}" />
					<e:text> ~ </e:text>
					<e:inputDate id="CMS_MAPPING_END_DATE" name="CMS_MAPPING_END_DATE" value="${toDate }" width="${inputDateWidth}" datePicker="true" required="${form_CMS_MAPPING_END_DATE_R}" disabled="${form_CMS_MAPPING_END_DATE_D}" readOnly="${form_CMS_MAPPING_END_DATE_RO}" />
				</e:field>
