    ...
    <script>
        ...
        function init() {
            ...
            grid.cellClickEvent(function(rowId, colId, value) {
                ...
                } else if(colId === "CUBL_NM") {
                    // if(grid.getCellValue(rowId, "CUBL_NM") != '' && grid.getCellValue(rowId, "CUBL_NM") != null) {
                    if(grid.getCellValue(rowId, "TAX_NUM") != '') {
                        alert("세금계산서 발행된 건은 수정 불가 입니다.");
                        return;
                    }
                        param = {
                            callBackFunction: "callbackGridCUBL_NM",
                            CUST_CD  : grid.getCellValue(rowId, "CUST_CD"),
            ...
            */
            ...
            /*
            grid.setColMerge(["PR_SUBJECT","DEAL_CD","PR_TYPE","CUST_CD","CUST_NM", "PLANT_NM","DEPT_NM", "CPO_USER_NM", "CPO_NO", "CPO_SEQ", "PO_NO", "PO_SEQ",
        ...
        function doSearch() {
            ...
            store.setGrid([grid]);
            store.load(baseUrl + "py01020_doSearch", function () {
                EVF.V("TOT_AMT", "0 원");
                EVF.V("TOT_GR_AMT", "0 원");
                EVF.V("TOT_PO_AMT", "0 원");
                EVF.V("MARGIN_AMT", "0 원");
                ...
                if(grid.getRowCount() == 0) {
                    ...
                    for( var i = 0; i < rows.length; i++ ) {
                        ...
                        TOT_AMT += Number(rows[i].GR_ITEM_AMT);
                    }
        ...
        function doResultInfoSave() {
            ...
            if(!confirm("${PY01_020_035}")) { return; }

            store.setGrid([grid]);
            store.getGridData(grid, "sel");
            store.load(baseUrl + "doResultInfoSave", function() {
        ...
        function doSave() {
            ...
            for(var i in selRowValue) {
                ...
                if(row.TAX_NUM !== "") {
                    return alert("${PY01_020_023}");
                }
            }

            if(!confirm("${PY01_020_021}")) { return; }

            store.setGrid([grid]);
            store.getGridData(grid, "sel");
            store.load(baseUrl + "py01020_doSave", function() {
                alert(this.getResponseMessage());
                doSearch();
            });
        }
...
//            if(!grid.validate().flag) { return alert(grid.validate().msg); }
            ...
            if(!confirm("${PY01_020_004}")) { return; }

            store.setGrid([grid]);
            store.getGridData(grid, "sel");
            store.load(baseUrl + "py01020_doDelete", function() {
                alert(this.getResponseMessage());
                doSearch();
            });
        }
        ...
        function doConfirm() {
            ...
            for( var i = 0; i < rows.length; i++ ) {
                if(rows[i].CUST_CONFIRM_FLAG === "1") {
                    return alert("${PY01_020_018}");
                }
            }

            if(!confirm("${PY01_020_016}")) { return; }

            store.setGrid([grid]);
            store.getGridData(grid, "sel");
            store.load(baseUrl + "py01020_doConfirm", function() {
                alert(this.getResponseMessage());
                doSearch();
            });
        }

        function doConfirmCancle() {
            var store = new EVF.Store();
            ...
            for( var i = 0; i < rows.length; i++ ) {
                if(rows[i].TAX_NUM != "") {
                    return alert("${PY01_020_026}");
                }
            }

            if(!confirm("${PY01_020_025}")) { return; }

            store.setGrid([grid]);
            store.getGridData(grid, "sel");
            store.load(baseUrl + "py01020_doConfirmCancle", function() {
                alert(this.getResponseMessage());
                doSearch();
            });
        }
        ...
        function doTaxCreate() {
            ...
            if(chkTax == "N"){
                if(!confirm("${PY01_020_032}")) {
                	return;
                } else {
                    store.setGrid([grid]);
                    store.getGridData(grid, "sel");
                    store.load(baseUrl + "py01020_doTaxCreate", function() {
                        alert(this.getResponseMessage());
                        doSearch();
                    });
                }
            } else {
                if(!confirm("${PY01_020_009}")) {
                	return;
                } else {
                    store.setGrid([grid]);
                    store.getGridData(grid, "sel");
        ...
        function onSearchPlant() {
            if( EVF.V("CUST_CD") == "" ) return alert("${PY01_020_031}");
            var param = {
        ...
        function onIconClickCUBL_SQ() {
            var param = {
                callBackFunction: "callbackCUBL_SQ",
        ...
        function callbackCUBL_SQ(data) {
            EVF.V("CUBL_SQ", data.CUBL_SQ);
            EVF.V("CUBL_NM", data.CUBL_NM);
        }
		...
		function doApply() {
			...
			for(var i in rowIds) {
				...
				grid.setCellValue(rowIds[i],'RESULT_DEAL_CD', EVF.C("RESULT_DEAL_CD").getValue());
				grid.setCellValue(rowIds[i],'RESULT_ITEM_GUBUN', EVF.C("RESULT_ITEM_GUBUN").getValue());
    ...
    <e:window id="PY01_020" onReady="init" initData="${initData}" title="${fullScreenName}">
        <e:searchPanel id="form" title="${form_CAPTION_N}" labelWidth="150px" labelAlign="${labelAlign}" columnCount="3" useTitleBar="false" onEnter="doSearch">
            ...
            <e:inputHidden id="SELL_TAX_TYPE" name="SELL_TAX_TYPE" value="500"/>
        </e:searchPanel>

        <e:panel width="55%" visible="true">
			...
			<e:select id="RESULT_DEAL_CD" name="RESULT_DEAL_CD" value="" options="${resultDealCdOptions}" width="${form_RESULT_DEAL_CD_W}" disabled="${form_RESULT_DEAL_CD_D}" readOnly="${form_RESULT_DEAL_CD_RO}" required="${form_RESULT_DEAL_CD_R}" placeHolder="" usePlaceHolder="false"/>
			<e:text>&nbsp;</e:text>
			<e:select id="RESULT_ITEM_GUBUN" name="RESULT_ITEM_GUBUN" value="" options="${resultItemGubunOptions}" width="${form_RESULT_ITEM_GUBUN_W}" disabled="${form_RESULT_ITEM_GUBUN_D}" readOnly="${form_RESULT_ITEM_GUBUN_RO}" required="${form_RESULT_ITEM_GUBUN_R}" placeHolder="" usePlaceHolder="false"/>
			<e:text>&nbsp;</e:text>
			<e:button id="doApply" name="doApply" label="${doApply_N}" onClick="doApply" disabled="${doApply_D}" visible="${doApply_V}"/>
			<e:text>&nbsp;</e:text>
			<e:button id="doResultInfoSave" name="doResultInfoSave" label="${doResultInfoSave_N}" onClick="doResultInfoSave" disabled="${doResultInfoSave_D}" visible="${doResultInfoSave_V}"/>
