...
<e:ui locale="${ses.countryCd}" lang="${ses.langCd}" dateFmt="${ses.dateFormat}">
    <script>
        ...
        function init() {
            ...
            // grid._gvo.setFixedOptions({colCount: 4});
//            grid.freezeCol("DEPT_NM");
        ...
        function doSearch() {
            ...
            store.setGrid([grid]);
            store.load(baseUrl + "py01010_doSearch", function () {
                ...
                if(grid.getRowCount() == 0) {
                    ...
                    for( var i = 0; i < rows.length; i++ ) {
                        TOT_AMT += Number(grid.getCellValue(rows[i], "GR_AMT"));
                        //var GR_ITEM_AMT = grid.getCellValue(rows[i], "GR_ITEM_AMT");
                        var GR_ITEM_AMT = grid.getCellValue(rows[i], "GR_AMT");

                        var PO_ITEM_AMT = grid.getCellValue(rows[i], "PO_ITEM_AMT");

                        var PROFIT_RATE = everMath.round_float(((GR_ITEM_AMT - PO_ITEM_AMT) / GR_ITEM_AMT) * 100, 1);

//                        grid.setCellValue(rows[i], "PROFIT_RATE", PROFIT_RATE, false);
                    }
...
//                    var cpoItemAmt = Number(grid._gvo.getSummary('GR_ITEM_AMT', 'SUM'));
                    ...
                    var cpoItemAmt = Number(grid._gvo.getSummary('GR_AMT', 'SUM'));
        ...
        function doSaveConfirm() {
            ...
            if(!confirm("${PY01_010_002}")) { return; }

            store.setGrid([grid]);
            store.getGridData(grid, "sel");
            store.load(baseUrl + "py01010_doSaveConfirm", function() {
        ...
        function onSearchPlant() {
            if( EVF.V("CUST_CD") == "" ) return alert("${PY01_010_017}");
            var param = {
    ...
    <e:window id="PY01_010" onReady="init" initData="${initData}" title="${fullScreenName}">
