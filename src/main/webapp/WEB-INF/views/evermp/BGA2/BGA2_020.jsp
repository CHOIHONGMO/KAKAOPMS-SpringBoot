...
/*
        function callbackGridREQ_TEXT(data) {                                   -- 주문요청사항
        ...
        function doSearch() {
            ...
            store.load(baseUrl + "bga2020_doSearch", function () {
        ...
        function doGiCancel() {
            ...
            if(!confirm("${BGA2_020_003}")) { return; }

            store.setGrid([grid]);
            store.getGridData(grid, "sel");
            store.load(baseUrl + "BGA2020_doGiCancel", function() {
		...
		function onSearchPlant() {
			if( EVF.V("CUST_CD") == "" ) return alert("${BGA2_020_021}");
			var param = {
        ...
        function doPrint() {
	        ...
	        for (var j in pouniq) {
		        ...
		        for( var k in rowIds ) {
		        	if(custCd_recipientNm == grid.getCellValue(rowIds[k], "CUST_CD") + grid.getCellValue(rowIds[k], "RECIPIENT_NM") + grid.getCellValue(rowIds[i], "CUBL_SQ")) {
				        ...
				        pouniqStr += grid.getCellValue(rowIds[k], "GI_NO") + grid.getCellValue(rowIds[k], "GI_SEQ") + ","
			        }
		        }
	        ...
	        url = oz80_url + "/ozhviewer/BGA2_020.jsp";
    ...
    <e:window id="BGA2_020" onReady="init" initData="${initData}" title="${fullScreenName}">
            ...
            <e:row>
				<e:label for="START_DATE">
                    <e:select id="START_DATE_COMBO" name="START_DATE_COMBO" width="99" disabled="false" readOnly="false" required="false" usePlaceHolder="false">
                        <e:option text="${BGA2_020_0009}" value="PO_DATE"/>
                    </e:select>
                </e:label>
				<e:field>
        ...
        <e:buttonBar align="right" width="100%">
			...
			<e:button id="doGiCancel" name="doGiCancel" label="${doGiCancel_N}" onClick="doGiCancel" disabled="${doGiCancel_D}" visible="${doGiCancel_V}"/>
		</e:buttonBar>
