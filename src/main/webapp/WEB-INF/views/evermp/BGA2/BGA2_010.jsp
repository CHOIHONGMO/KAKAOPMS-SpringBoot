    ...
    <script>
        ...
        function init() {
            ...
            if('${form.autoSearchFlag}' == 'Y') {
                ...
                store.setGrid([grid]);
                store.load(baseUrl + "bga2010_doSearch", function () {
            ...
            grid.cellClickEvent(function(rowId, colId, value, iRow, iCol) {
	        	...
	        	if(colId === "CPO_USER_NM") {
					...
					everPopup.openPopupByScreenId('commonFileAttach', 650, 310, param);
                } else if(colId == "SG_CTRL_USER_NM") { // 진행관리담당자
            ...
            grid.cellChangeEvent(function(rowId, colId, iRow, iCol, value, oldValue) {
				if(colId === "GI_QTY" && value !== oldValue) {
				    if(value == 0) {
                        alert("${BGA2_010_009}");
                        grid.setCellValue(rowId, "GI_QTY", oldValue);
                        grid.checkRow(rowId, false);
                        return;
					}

				    var AV_GI_QTY =  Number(grid.getCellValue(rowId, "AV_GI_QTY"));
					...
					var GI_AMT = CPO_UNIT_PRICE * value;

					grid.setCellValue(rowId, "GI_AMT", GI_AMT);
...
/*            grid.setColMerge(["CUST_NM", "DEPT_NM", "CPO_USER_NM", "REF_MNG_NO", "CPO_NO", "CPO_SEQ", "PO_NO", "PO_SEQ", "CUST_ITEM_CD", "ITEM_CD", "ITEM_DESC", "ITEM_SPEC",
        ...
        function doGiSave() {
            ...
            if(!confirm("${BGA2_010_004}")) { return; }

            store.setGrid([grid]);
            store.getGridData(grid, "sel");
            store.load(baseUrl + "bga2010_doGiSave", function() {
        ...
        function doAllApply() {
            ...
            for( var i in rowIds ) {
                grid.setCellValue(rowIds[i], 'GI_DATE', delyDate);
            }
        }

    </script>

    <e:window id="BGA2_010" onReady="init" initData="${initData}" title="${fullScreenName}">
            ...
            <e:row>
				<e:label for="START_DATE">
                    <e:select id="START_DATE_COMBO" name="START_DATE_COMBO" width="99" disabled="false" readOnly="false" required="false" usePlaceHolder="false">
                        <e:option text="${BGA2_010_0009}" value="PO_DATE"/>
                    </e:select>
                </e:label>
				<e:field>
		...
		<e:panel height="fit" width="50%">
			<e:text style="font-weight: bold;">●&nbsp;${form_GI_DATE_N} &nbsp;:&nbsp; </e:text>
			<e:inputDate id="GI_DATE" name="GI_DATE" value="${END_DATE}" width="${inputDateWidth}" datePicker="true" required="${form_GI_DATE_R}" disabled="${form_GI_DATE_D}" readOnly="${form_GI_DATE_RO}" />
			<e:text>&nbsp;</e:text>
		...
		<e:panel height="fit" width="50%">
			<e:buttonBar align="right" width="100%">
				...
				<e:button id="doGiSave" name="doGiSave" label="${doGiSave_N}" onClick="doGiSave" disabled="${doGiSave_D}" visible="${doGiSave_V}"/>
			</e:buttonBar>
		</e:panel>
