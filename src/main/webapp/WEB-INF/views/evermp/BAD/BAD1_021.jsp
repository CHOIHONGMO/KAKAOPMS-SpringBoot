...
<e:ui locale="${ses.countryCd}" lang="${ses.langCd}" dateFmt="${ses.dateFormat}">
    <script type="text/javascript">
        ...
        function doSave() {
            ...
            if(EVF.V("ID_CHECK")==""){
                return alert("${BAD1_021_002}");
            }

            if(!confirm(btnMsg)) { return; }

            store.load(baseUrl + 'bad1021_doSave', function () {
                ...
                var param = {
                    ...
                    window.location.href = '/evermp/BAD/BAD1/BAD1_021/view.so?' + $.param(param);
            });
        }
       ...
       function doIdcheck() {
           ...
           if( EVF.C("USER_ID").getValue() == "" ){
               ...
               var store = new EVF.Store();
               store.load(baseUrl + 'bad1021_doCheckUserId', function() {
                   if(this.getParameter("POSSIBLE_FLAG") == "N") {
                       EVF.V("ID_CHECK","");
                       return alert("${BAD1_021_006}");
                   }else{
                       EVF.V("ID_CHECK","Y");
                       alert("${BAD1_021_001}");
                   }
       ...
       function IdcheckId(){
        ...
        function checkCellNo() {
            ...
            if(checkType == "RECIPIENT_CELL_NUM") {
    ...
    <e:window id="BAD1_021" onReady="init" initData="${initData}" title="${fullScreenName}" breadCrumbs="${breadCrumb }">

         <e:buttonBar id="buttonTopBottom" align="right" width="100%" title="${BAD1_021_CAPTION1 }">
             <e:button id="Insert" name="Insert" label="${Insert_N}" onClick="doSave" disabled="${Insert_D}" visible="${Insert_V}" data="I"/>
             <e:button id="Update" name="Update" label="${Update_N}" onClick="doSave" disabled="${Update_D}" visible="${Update_V}" data="U"/>
         </e:buttonBar>
        ...
        <e:searchPanel id="form1" title="${form_CAPTION_N }" labelWidth="${labelWidth}" width="100%" columnCount="2" useTitleBar="false">
            ...
            <e:inputHidden id="PLANT_CD" name="PLANT_CD" value="${formData.PLANT_CD}" />   <!-- 사업장코드 -->
            ...
            <e:row>
                ...
                <e:field>
                    <e:inputText id="COMPANY_CD" name="COMPANY_CD" value="${ses.companyCd }" width="${form_COMPANY_CD_W}" maxLength="${form_COMPANY_CD_M}" disabled="${form_COMPANY_CD_D}" readOnly="${form_COMPANY_CD_RO}" required="${form_COMPANY_CD_R}" />
                </e:field>
                <e:label for="COMPANY_NM" title="${form_COMPANY_NM_N}"></e:label>
                <e:field>
                    <e:inputText id="COMPANY_NM" style='ime-mode:inactive' name="COMPANY_NM" value="${ses.companyNm }" width="${form_COMPANY_NM_W}" maxLength="${form_COMPANY_NM_M}" disabled="${form_COMPANY_NM_D}" readOnly="${form_COMPANY_NM_RO}" required="${form_COMPANY_NM_R}" />
                </e:field>
            </e:row>
            <e:row>
            ...
            <e:row>
                ...
                <e:field>
                    <e:search id="DEPT_CD" name="DEPT_CD" value="${formData.DEPT_CD }" width="35%" maxLength="${form_DEPT_CD_M}" onIconClick="${param.detailView ? 'everCommon.blank' : 'deptCdSearch_I'}" disabled="${form_DEPT_CD_D}" readOnly="${form_DEPT_CD_RO}" required="${form_DEPT_CD_R}" />
                    <e:inputText id="DEPT_NM" style='ime-mode:inactive' name="DEPT_NM" value="${formData.DEPT_NM}" width="65%" maxLength="${form_DEPT_NM_M}" disabled="${form_DEPT_NM_D}" readOnly="true" required="${form_DEPT_NM_R}" />
                </e:field>
            </e:row>
            <e:row>
                <e:label for="TEL_NUM" title="${form_TEL_NUM_N}"></e:label>
                <e:field>
                    <e:inputText id="TEL_NUM" name="TEL_NUM"  placeHolder="${BAD1_021_INPUT_T2 }" value="${formData.TEL_NUM}" width="100%" maxLength="${form_TEL_NUM_M}" disabled="${form_TEL_NUM_D}" readOnly="${form_TEL_NUM_RO}" required="${form_TEL_NUM_R}" onChange="checkTelNo" data="TEL_NUM" />
                </e:field>
                <e:label for="FAX_NUM" title="${form_FAX_NUM_N}"></e:label>
                <e:field>
                    <e:inputText id="FAX_NUM" name="FAX_NUM" placeHolder="${BAD1_021_INPUT_T2 }" value="${formData.FAX_NUM}" width="100%" maxLength="${form_FAX_NUM_M}" disabled="${form_FAX_NUM_D}" readOnly="${form_FAX_NUM_RO}" required="${form_FAX_NUM_R}" onChange="checkTelNo" data="FAX_NUM" />
                </e:field>
            </e:row>
            <e:row>
                <e:label for="CELL_NUM" title="${form_CELL_NUM_N}"></e:label>
                <e:field>
                    <e:inputText id="CELL_NUM" name="CELL_NUM"  placeHolder="${BAD1_021_INPUT_T2 }" value="${formData.CELL_NUM}" width="100%" maxLength="${form_CELL_NUM_M}" disabled="${form_CELL_NUM_D}" readOnly="${form_CELL_NUM_RO}" required="${form_CELL_NUM_R}" onChange="checkCellNo" data="CELL_NUM" />
                </e:field>
            ...
            <e:row>
                ...
                <e:field>
                    <e:search id="CHIEF_USER_NM" name="CHIEF_USER_NM" value="${formData.CHIEF_USER_NM}" width="${form_CHIEF_USER_NM_W}" maxLength="${form_CHIEF_USER_NM_M}" onIconClick="${param.detailView ? 'everCommon.blank' : 'doUserSearch'}" disabled="${form_CHIEF_USER_NM_D}" readOnly="${form_CHIEF_USER_NM_RO}" required="${form_CHIEF_USER_NM_R}" onChange="clearUser" />
            ...
            <e:row>
                ...
                <e:field>
                    <e:search id="BUDGET_DEPT_CD" name="BUDGET_DEPT_CD" value="${formData.BUDGET_DEPT_CD }" width="40%" maxLength="${form_BUDGET_DEPT_CD_M}" onIconClick="${param.detailView ? 'everCommon.blank' : 'deptCdSearch_2'}" disabled="${form_BUDGET_DEPT_CD_D}" readOnly="${form_BUDGET_DEPT_CD_RO}" required="${form_BUDGET_DEPT_CD_R}" />
            ...
            <e:row>
                ...
                <e:field>
                    <e:select id="BUDGET_FLAG" name="BUDGET_FLAG" value="${formData.BUDGET_FLAG }" options="${budgetFlagOptions }" width="${form_BUDGET_FLAG_W}" disabled="${form_BUDGET_FLAG_D}" readOnly="${form_BUDGET_FLAG_RO}" required="${form_BUDGET_FLAG_R}" placeHolder="" />
                </e:field>
            ...
            <e:row>
                ...
                <e:field>
                    <e:select id="FINANCIAL_FLAG" name="FINANCIAL_FLAG" value="${formData.FINANCIAL_FLAG }" options="${financialFlagOptions }" width="${form_FINANCIAL_FLAG_W}" disabled="${form_FINANCIAL_FLAG_D}" readOnly="${form_FINANCIAL_FLAG_RO}" required="${form_FINANCIAL_FLAG_R}" placeHolder="" />
                </e:field>
                <e:label for="GR_FLAG" title="${form_GR_FLAG_N}" />
                <e:field>
                    <e:select id="GR_FLAG" name="GR_FLAG" value="${formData.GR_FLAG }" options="${grFlagOptions }" width="${form_GR_FLAG_W}" disabled="${form_GR_FLAG_D}" readOnly="${form_GR_FLAG_RO}" required="${form_GR_FLAG_R}" placeHolder="" />
                </e:field>
            </e:row>
        </e:searchPanel>
		...
		<div id="CustCsdm" style="display: block;">
            <e:searchPanel id="custform5" title="${form_CAPTION_N }" labelWidth="${labelWidth}" width="100%" columnCount="2" useTitleBar="false">
                <e:panel id="custanel5" height="fit" width="30%">
                    <e:title title="${BAD1_021_CAPTION6 }" depth="1"/>
                </e:panel>
                <e:row>
                    <e:label for="CSDM_SEQ" title="${form_CSDM_SEQ_N}"></e:label>
                    <e:field>
                        <e:search id="CSDM_SEQ" name="CSDM_SEQ" value="${formData.CSDM_SEQ }" width="30%" maxLength="${form_CSDM_SEQ_M}" onIconClick="${param.detailView ? 'everCommon.blank' : 'csdmCdSearch'}" disabled="${form_CSDM_SEQ_D}" readOnly="${form_CSDM_SEQ_RO}" required="${form_CSDM_SEQ_R}" />
                        <e:inputText id="DELY_NM" style='ime-mode:inactive' name="DELY_NM" value="${formData.DELY_NM}" width="70%" maxLength="${form_DELY_NM_M}" disabled="${form_DELY_NM_D}" readOnly="true" required="${form_DELY_NM_R}" />
                    </e:field>
                ...
                <e:row>
                    ...
                    <e:field>
                        <e:inputText id="DELY_RECIPIENT_FAX_NUM" name="DELY_RECIPIENT_FAX_NUM" value="${formData.DELY_RECIPIENT_FAX_NUM}" width="${form_DELY_RECIPIENT_FAX_NUM_W}" maxLength="${form_DELY_RECIPIENT_FAX_NUM_M}" disabled="${form_DELY_RECIPIENT_FAX_NUM_D}" readOnly="${form_DELY_RECIPIENT_FAX_NUM_RO}" required="${form_DELY_RECIPIENT_FAX_NUM_R}" />
                    </e:field>
                </e:row>
                <e:row>
        ...
        <div id="CustCubl" style="display: block;">
            <e:searchPanel id="custform6" title="${form_CAPTION_N }" labelWidth="${labelWidth}" width="100%" columnCount="2" useTitleBar="false">
                <e:panel id="custanel6" height="fit" width="30%">
                    <e:title title="${BAD1_021_CAPTION7 }" depth="1"/>
                </e:panel>
                <e:row>
                    <e:label for="CUBL_SQ" title="${form_CUBL_SQ_N}"></e:label>
                    <e:field>
                        <e:search id="CUBL_SQ" name="CUBL_SQ" value="${formData.CUBL_SQ }" width="30%" maxLength="${form_CUBL_SQ_M}" onIconClick="${param.detailView ? 'everCommon.blank' : 'cublCdSearch'}" disabled="${form_CUBL_SQ_D}" readOnly="${form_CUBL_SQ_RO}" required="${form_CUBL_SQ_R}" />
                        <e:inputText id="CUBL_NM" style='ime-mode:inactive' name="CUBL_NM" value="${formData.CUBL_NM}" width="70%" maxLength="${form_CUBL_NM_M}" disabled="${form_CUBL_NM_D}" readOnly="true" required="${form_CUBL_NM_R}" />
                    </e:field>
                ...
                <e:row>
                    <e:label for="CUBL_USER_TEL_NUM" title="${form_CUBL_USER_TEL_NUM_N}" />
                    <e:field>
                        <e:inputText id="CUBL_USER_TEL_NUM" name="CUBL_USER_TEL_NUM" value="${formData.CUBL_USER_TEL_NUM}" width="${form_CUBL_USER_TEL_NUM_W}" maxLength="${form_CUBL_USER_TEL_NUM_M}" disabled="${form_CUBL_USER_TEL_NUM_D}" readOnly="${form_CUBL_USER_TEL_NUM_RO}" required="${form_CUBL_USER_TEL_NUM_R}" />

                    </e:field>
                    <e:label for="CUBL_USER_FAX_NUM" title="${form_CUBL_USER_FAX_NUM_N}" />
                    <e:field>
                        <e:inputText id="CUBL_USER_FAX_NUM" name="CUBL_USER_FAX_NUM" value="${formData.CUBL_USER_FAX_NUM}" width="${form_CUBL_USER_FAX_NUME_W}" maxLength="${form_CUBL_USER_FAX_NUM_M}" disabled="${form_CUBL_USER_FAX_NUM_D}" readOnly="${form_CUBL_USER_FAX_NUM_RO}" required="${form_CUBL_USER_FAX_NUM_R}" />
                    </e:field>
                </e:row>
                <e:row>
