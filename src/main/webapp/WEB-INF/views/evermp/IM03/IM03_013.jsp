...
<e:ui locale="${ses.countryCd}" lang="${ses.langCd}" dateFmt="${ses.dateFormat}">
    <script>
        ...
        function init() {
            ...
            if(json.length != 0) {
                ...
                store.load(baseUrl + "IM03009_doSearchCandidate", function() {
                    gridR.checkAll(true);
                });
            }
            ...
            gridL.setProperty('multiSelect', (${param.detailView} ? false : true));
            gridR.setProperty('multiSelect', (${param.detailView} ? false : true));

            if(!EVF.isEmpty("${param.CUST_CD_LIST}")){
                doSettingparma();
            }
        }
        ...
        function doSettingparma(){
            var store = new EVF.Store();
            store.setGrid([gridR]);
            store.load(baseUrl + "IM03009_doSetGridR", function() {
                gridR.checkAll(true);
            });
        }
        ...
        function doSelect() {
            ...
            opener.window['${param.callBackFunction}'](JSON.stringify(selAllRowDataR), '${param.rowId}');
            doClose();
        }
    ...
    <e:window id="IM03_013" onReady="init" initData="${initData}" title="${fullScreenName}">
        <e:searchPanel id="form" title="${msg.M9999 }" columnCount="2" labelWidth="135" onEnter="doSearch
" useTitleBar="false">
            <e:row>
                <e:inputHidden id="CUST_CD_LIST" name="CUST_CD_LIST" value="${param.CUST_CD_LIST}" />
        ...
        <e:panel height="fit" width="100%">
            <e:panel width="49%">
                <e:title title="${IM03_013_0002 }" />
                <e:gridPanel id="gridL" name="gridL" height="fit" width="100%" gridType="${_gridType}" readOnly="${param.detailView}" columnDef="${gridInfos.grid.gridColData}" />
            </e:panel>
