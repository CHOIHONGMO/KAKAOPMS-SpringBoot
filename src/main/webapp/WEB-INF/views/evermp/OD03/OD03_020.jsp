...
// 					}
			...
			*/
            ...
            grid.setColMerge(["DELY_NM","CSDM_SEQ","VENDOR_CD","PO_DATE","DEAL_CD","PR_TYPE","CUST_CD","CUST_NM","CUST_NM","PLANT_NM","DEPT_NM","PR_SUBJECT", "CPO_USER_NM", "REF_MNG_NO", "CPO_NO", "CPO_SEQ", "PO_NO", "PO_SEQ", "CUST_ITEM_CD", "ITEM_CD", "ITEM_DESC", "ITEM_SPEC",
                "MAKER_NM", "MAKER_PART_NO", "BRAND_NM", "ORIGIN_NM", "UNIT_CD", "CPO_QTY"
                ,'CPO_UNIT_PRICE'
                ,'CPO_ITEM_AMT'
                ,'PO_UNIT_PRICE'
                ,'PO_ITEM_AMT'
        ...
        function doSearch() {
            ...
            store.load(baseUrl + "od03020_doSearch", function () {
        ...
        function doGrCancel() {
            ...
            var rowIds = grid.getSelRowId();
                ...
                */
                if(grid.getCellValue(rowIds[i], 'IF_GR_NO') != "") {
                	return alert("${OD03_020_021}");
                }
                if(grid.getCellValue(rowIds[i], 'CLOSING_YN') == "Y" || grid.getCellValue(rowIds[i], 'GR_CLOSE_YN') == "1") {
                	return alert("${OD03_020_002}");
                }
           	}

            if(!confirm("${OD03_020_003}")) { return; }
		...
		function onSearchPlant() {
			if( EVF.V("CUST_CD") == "" ) return alert("${OD03_020_019}");
			var param = {
    ...
    <e:window id="OD03_020" onReady="init" initData="${initData}" title="${fullScreenName}">
            ...
            <e:row>
				<e:label for="START_DATE">
                    <e:select id="START_DATE_COMBO" name="START_DATE_COMBO" width="99" disabled="false" readOnly="false" required="false" usePlaceHolder="false">
                        <e:option text="${OD03_020_0009}" value="CPO_DATE"/>
						<e:option text="${OD03_020_020}" value="PO_DATE"/>
						<e:option text="${OD03_020_018}" value="GR_DATE"/>
                        <e:option text="${OD03_020_0010}" value="HOPE_DUE_DATE"/>
                    </e:select>
                </e:label>
				<e:field>
