...
<e:ui locale="${ses.countryCd}" lang="${ses.langCd}" dateFmt="${ses.dateFormat}">
    <script>
        ...
        function init() {
            ...
            EVF.C("PROGRESS_CD").removeOption("1100");
            EVF.C("PROGRESS_CD").removeOption("1150");
            ...
            grid.cellClickEvent(function(rowIdx, colIdx, value, iRow, iCol) {
                ...
                if(colIdx == "multiSelect") {
                    ...
                    everPopup.openModalPopup(url, 600, 360, param);
                } else if(colIdx == 'AM_USER_NM') { // 진행관리담당자
                    ...
                    everPopup.openPopupByScreenId("commonFileAttach", 650, 310, param);
                } else if (colIdx == "CHG_YN") {
                	if(grid.getCellValue(rowIdx, "CHG_YN")!='변경') return;
                    param = {
                    		APP_DOC_NUM : grid.getCellValue(rowIdx, "APP_DOC_NO"),
...
//            grid.setColIconify("REQ_TEXT", "REQ_TEXT", "comment", false);
        ...
        function doSearch() {
            ...
            store.load(baseUrl + "OD01_010/doSearch", function () {
...
//                        }
            ...
            new EVF.Mask().mask();
            $.ajax({
                url: baseUrl + "OD01_010/doSearch",
                data: $('input').serialize().replace(/multiselect_/gi, '') + "&autoSearchFlag="+autoSearchFlag + "&callType="+callType + "&relatYn="+'${form.relatYn}',
                success: function(data) {
                    ...
                    if (rowCount == 0) {
                        ...
                        gridJsonData = data;
                        grid._gdp.fillJsonData(data, {count: 10});

                        // Excel Download를 위해 전역변수에 Data를 담아둔다.
                        excelData = JSON.parse(gridJsonData);
...
//                            }
        ...
        function doChangeOrder() {
            ...
            for(var i = 0; i < rowIds.length; i++) {
                ...
                if(!(selectedData.PROGRESS_CD == '38' || selectedData.PROGRESS_CD == '40')) {
                    return alert("${OD01_010_0006}"); // 진행상태를 확인하여 주시기 바랍니다.
                }
            }
        ...
        function doCancle() {
            ...
            for( var i = 0; i < rows.length; i++ ) {
                if(rows[i].PROGRESS_CD !== "30") {
                    return alert("${OD01_010_006}");
                }
                if( cpoNo != "" && cpoNo != rows[i].CPO_NO ){
                    return alert("${OD01_010_009}");
                }
                cpoNo = rows[i].CPO_NO;
            }

            if(!confirm("${OD01_010_007}")) { return; }
        ...
        function doCanclePO() {
            ...
            for( var i = 0; i < rows.length; i++ ) {
                if(rows[i].PROGRESS_CD !== "38") {
                    return alert("${OD01_010_005}");
                }
            }

            if(!confirm("${OD01_010_008}")) { return; }
        ...
        function onSearchPlant() {
            if( EVF.V("CUST_CD") == "" ) return alert("${OD01_010_001}");
            var param = {
    ...
    <e:window id="OD01_010" onReady="init" initData="${initData}" title="${fullScreenName}">
            ...
            <e:row>
                ...
                <e:label for="START_DATE">
                    <e:select id="START_DATE_COMBO" name="START_DATE_COMBO" width="99" disabled="false" readOnly="false" required="false" usePlaceHolder="false">
                        <e:option text="${OD01_010_0001}" value="CPO_DATE"/>
                        <e:option text="${OD01_010_0002}" value="HOPE_DUE_DATE"/>
                    </e:select>
                </e:label>
                <e:field>
        ...
        <e:panel width="40%">
            ...
            <!-- e:button id="doExcelDown" name="doExcelDown" label="${doExcelDown_N}" onClick="doExcelDown" disabled="${doExcelDown_D}" visible="${doExcelDown_V}"/-->
            </e:buttonBar>
