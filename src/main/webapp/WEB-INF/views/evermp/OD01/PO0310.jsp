            ...
            */
            ...
            grid.cellClickEvent(function(rowId, colIdx, value) {
                ...
                if(colIdx == 'CPO_USER_NM') {
                    ...
                    everPopup.openPopupByScreenId("BYM1_062", 680, 180, param);
                    ...
                    everPopup.openPopupByScreenId("PO0311", 1000, 500, param);
                }else if (colIdx == "CUR_QTY") {
                    param  = {
                            rowId            : rowId,
                            ITEM_CD         : grid.getCellValue(rowId, 'ITEM_CD'),
                            detailView      : true
...
//                grid.setCellRequired(rowIdx, "CONFIRM_REQ_RMK", true);
        ...
        function onSearchPlant() {
            if( EVF.V("CUST_CD") == "" ) return alert("${PO0310_001}");
            var param = {
        ...
        function doSearch() {
            ...
            store.load(baseUrl + "PO0310_doSearch", function () {
                var footerRateZero = {
                    "styles": {
                        "textAlignment": "far",
                        "suffix": " ",
...
//                        grid.setCellRequired(rowIds[i], "CONFIRM_REQ_RMK", true);
        ...
        function doReqConfirm() {
            ...
            for(var i in rowIds) {
                ...
                if(grid.getCellValue(rowIds[i], 'AM_USER_ID') != "${ses.userId}" && '${ses.ctrlCd}'.indexOf('M100') < 0) {
                    ...
                    return alert("${PO0310_0008}");
                }
                */

                if(grid.getCellValue(rowIds[i], "PROGRESS_CD") != "30") {
                    grid.checkRow(rowIds[i], false);
                    return alert("${PO0310_006}");
                }
            ...
            if(checkYN == "Y") {
                if(!confirm('${PO0310_0027}')) { return; }
            } else {
                if(!confirm('${PO0310_0013}')) { return; }
            }
            ...
            for(var i in rowIds) {
                ...
                */
             	if(grid.getCellValue(rowIds[i], "CHANGE_YN") == 'Y'){
            		if(!confirm('${PO0310_0047}')) { return; };
                }

            	if(grid.getCellValue(rowIds[i], "PROGRESS_CD") != "5100") {
                    grid.checkRow(rowIds[i], false);
                    return alert("${PO0310_0029}");
                }

            	if(grid.getCellValue(rowIds[i], "SIGN_STATUS") == "P") {
                    grid.checkRow(rowIds[i], false);
                    return alert("${PO0310_0044}");
                }
            	if(grid.getCellValue(rowIds[i], 'CPO_QTY') > grid.getCellValue(rowIds[i], 'CUR_QTY')) {
                    confirm("${PO0310_0046}"); break;
                }
            }

            if(!confirm('${PO0310_0031}')) { return; }
            var store = new EVF.Store();
            store.setGrid([grid]);
            store.getGridData(grid, 'sel');
            store.setParameter("PROGRESS_CD", '6100'); // 협력사 전송
            store.load(baseUrl + 'PO0310_doPoConfirm', function() {
                alert(this.getResponseMessage());
                doSearch();
            });
        }
            ...
            for(var i in rowIds) {
                ...
                */
                if(grid.getCellValue(rowIds[i], 'PROGRESS_CD') != "5100") { // 접수완료 상태일때만 이관 가능
                    return alert("${PO0310_0006}");
                }
            	if(grid.getCellValue(rowIds[i], "SIGN_STATUS") == "P") {
                    grid.checkRow(rowIds[i], false);
                    return alert("${PO0310_0045}");
                }
            }

            if(!confirm('${PO0310_0019}')) { return; }
        ...
        function doChangeOrder() {
            ...
            for(var i = 0; i < rowIds.length; i++) {
                ...
                if(selectedData.PROGRESS_CD == '10' || selectedData.PROGRESS_CD == '20' || selectedData.PROGRESS_CD == '35') {
                    return alert("${PO0310_0006}"); // 진행상태를 확인하여 주시기 바랍니다.
                }
            }
        ...
        function doCancle() {
            ...
            for( var i = 0; i < rows.length; i++ ) {
                ...
                if( cpoNo != "" && cpoNo != rows[i].CPO_NO ){
                    return alert("${PO0310_009}");
                }
                cpoNo = rows[i].CPO_NO;
            }

            if(!confirm("${PO0310_007}")) { return; }
        ...
        function doTransferCtrl() {
            ...
            if(EVF.isEmpty(EVF.C("sAM_USER_ID").getValue())) { return alert("${PO0310_0007}"); }
        ...
        function setAmChangeRMK(data) {
            ...
            store.load(baseUrl + 'po0310_doTransferCtrl', function() {
                alert(this.getResponseMessage());
                doSearch();
            });
        }
		...
		function doUpoModify() {
            ...
            for(var i in selRowId) {
                ...
                if(signStatus=='P') {
                	return alert('${PO0310_0043}');
                }
            }

            if (!confirm("${PO0310_0042}")) return;
        ...
        function goApproval(formData, gridData, attachData) {
            ...
            store.load(baseUrl + 'PO0310_poItemChange', function () {
                alert(this.getResponseMessage());
                doSearch();
            });
        }
    ...
    <e:window id="PO0310" onReady="init" initData="${initData}" title="${fullScreenName}">
            ...
            <e:row>
                <e:label for="START_DATE">
                    <e:select id="START_DATE_COMBO" name="START_DATE_COMBO" width="99" disabled="false" readOnly="false" required="false" usePlaceHolder="false">
                        <e:option text="${PO0310_0001}" value="CPO_DATE"/>
                        <e:option text="${PO0310_0002}" value="HOPE_DUE_DATE"/>
                    </e:select>
                </e:label>
                <e:field>
