        ...
        function init() {
            ...
            grid.cellClickEvent(function(rowId, colIdx, value) {
                ...
                if(colIdx == 'CPO_USER_NM') {
                    if( grid.getCellValue(rowId, 'CPO_USER_NM') == '' ) return;
                    param = {
                        callbackFunction: "",
                    ...
                    if( grid.getCellValue(rowId, 'AM_USER_NM') == '' ) return;
                    param = {
                        callbackFunction: "",
...
//            grid.setColIconify("REQ_TEXT", "REQ_TEXT", "comment", false);
        ...
        function doSearch() {
            ...
            store.load(baseUrl + "OD01_001/doSearch", function () {
                var footerRateZero = {
                    "styles": {
                        "textAlignment": "far",
                        "suffix": " ",
        ...
        function doReqConfirm() {
            ...
            for(var i in rowIds) {
                /*
                if(grid.getCellValue(rowIds[i], 'AM_USER_NM') != '' && grid.getCellValue(rowIds[i], 'AM_USER_ID') != "${ses.userId}" && '${ses.ctrlCd}'.indexOf('M100') < 0) {
                    grid.checkRow(rowIds[i], false);
                    return alert("${OD01_001_0008}");
                }
                */

                if(grid.getCellValue(rowIds[i], "PROGRESS_CD") != "30") {
                    grid.checkRow(rowIds[i], false);
                    return alert("${OD01_001_006}");
                }
            ...
            if(checkYN == "Y") {
                if(!confirm('${OD01_001_0027}')) { return; }
            } else {
                if(!confirm('${OD01_001_0013}')) { return; }
            }
            ...
            for(var i in rowIds) {
                ...
                */
                if(grid.getCellValue(rowIds[i], "PROGRESS_CD") != "2100") {
                    grid.checkRow(rowIds[i], false);
                    return alert("${OD01_001_0029}"); // 진행상태가 [접수대기] 건만 처리 가능합니다.
                }
            }

            if(!confirm('${OD01_001_0031}')) { return; }

            var store = new EVF.Store();
            store.setGrid([grid]);
            store.getGridData(grid, 'sel');
            store.setParameter("PROGRESS_CD", '2200');
            store.load(baseUrl + 'od01001_doPoConfirm', function() {
                alert(this.getResponseMessage());
                doSearch();
            });
        }
        ...
        function doPoConfirmReject() {
            ...
            for(var i in rowIds) {
                /*
                if(grid.getCellValue(rowIds[i], 'AM_USER_NM') != '' && grid.getCellValue(rowIds[i], 'AM_USER_ID') != "${ses.userId}" && '${ses.ctrlCd}'.indexOf('M100') < 0) {
                    return alert("${OD01_001_0008}"); // 본인에게 지정된 건만 처리할 수 있습니다.
                }
                */

                if(grid.getCellValue(rowIds[i], 'PR_TYPE') != "C" && grid.getCellValue(rowIds[i], 'PROGRESS_CD') != "2100") { // 접수대기(2100) 상태일때만 주문반려 가능
                    return alert("${OD01_001_0006}"); // 진행상태를 확인하여 주시기 바랍니다.
                }
            }
            ...
            if(!confirm('${OD01_001_0019}')) { return; }
        ...
        function doSave() {
            ...
            if(!confirm("${OD01_001_0024}")) { return; }
        ...
        function doCancle() {
            ...
            for( var i = 0; i < rows.length; i++ ) {
                ...
                if( cpoNo != "" && cpoNo != rows[i].CPO_NO ){
                    return alert("${OD01_001_009}");
                }
                cpoNo = rows[i].CPO_NO;
            }

            if(!confirm("${OD01_001_007}")) { return; }
        ...
        function doTransferCtrl() {
            ...
            if(EVF.isEmpty(EVF.C("sAM_USER_ID").getValue())) { return alert("${OD01_001_0007}"); }
            ...
            for(var i in rowIds) {
                ...
                */
                if(grid.getCellValue(rowIds[i], 'PROGRESS_CD') != "2200") { // 접수완료 상태일때만 이관 가능
//                    return alert("${OD01_001_0006}");
                }
                changeRmk = grid.getCellValue(rowIds[i], 'AM_USER_CHANGE_RMK');
            }

            if(!confirm('${OD01_001_0009}')) { return; }
        ...
        <%--견적요청--%>
		function doRequestRFQ() {
			var value = grid.getSelRowValue()[0];

			if(!grid.isExistsSelRow()) { return EVF.alert("${msg.M0004}"); }
			for(var i in grid.getSelRowId()) {
				if(grid.getSelRowValue()[i].PROGRESS_CD != '2200') {
					return EVF.alert('${OD01_001_0039}');
				}
				if(!(grid.getSelRowValue()[i].CUST_CD == value.CUST_CD && grid.getSelRowValue()[i].PLANT_CD == value.PLANT_CD && grid.getSelRowValue()[i].CPO_USER_ID == value.CPO_USER_ID)) {
					return EVF.alert('${OD01_001_0041}');
				}
// 				if(grid.getSelRowValue()[i].PROGRESS_CD == '1200') {
// 					return EVF.alert('${msg.M0169}');
// 				}
// 				if(grid.getSelRowValue()[i].CUR != value.CUR) {
// 					return EVF.alert('${PR0340_0014}');
// 				}
				if(grid.getSelRowValue()[i].PR_TYPE != "C") {
					return EVF.alert('${OD01_001_0040}');
				}
			}
			...
			var param = {
				...
				PR_BUYER_CD 	: value.CUST_CD,
				PR_BUYER_NM 	: value.CUST_NM,
				PR_PLANT_CD 	: value.PLANT_CD,
				PR_PLANT_NM 	: value.PLANT_NM,
// 				PR_REQ_TYPE 	: value.PR_REQ_TYPE,
// 				PUR_ORG_CD   	: value.CUST_CD,
// 				CONT_REQ_FLAG 	: value.CONT_REQ_FLAG,
				detailView 		: false
			};
...
//	 				CONT_REQ_FLAG 	: value.CONT_REQ_FLAG,
        ...
        function onSearchPlant() {
            if( EVF.V("CUST_CD") == "" ) return alert("${OD01_001_001}");
            var param = {
    ...
    <e:window id="OD01_001" onReady="init" initData="${initData}" title="${fullScreenName}">
            ...
            <e:row>
                ...
                <e:label for="START_DATE">
                    <e:select id="START_DATE_COMBO" name="START_DATE_COMBO" width="99" disabled="false" readOnly="false" required="false" usePlaceHolder="false">
                        <e:option text="${OD01_001_0001}" value="CPO_DATE"/>
                        <e:option text="${OD01_001_0002}" value="HOPE_DUE_DATE"/>
                    </e:select>
                </e:label>
                <e:field>
        ...
        <e:buttonBar align="right" width="100%">
			...
			<e:button id="doRfq" name="doRfq" label="${doRfq_N}" onClick="doRequestRFQ" disabled="${doRfq_D}" visible="${doRfq_V}"/>
			<e:button id="doBd" name="doBd" label="${doBd_N}" onClick="doRequestRFX" disabled="${doBd_D}" visible="${doBd_V}"/>

            <!-- e:button id="doReqConfirm" name="doReqConfirm" label="${doReqConfirm_N}" onClick="doReqConfirm" disabled="${doReqConfirm_D}" visible="${doReqConfirm_V}"/-->
            <!-- e:button id="doSave" name="doSave" label="${doSave_N}" onClick="doSave" disabled="${doSave_D}" visible="${doSave_V}"/-->
            <!-- e:button id="doChangeOrder" name="doChangeOrder" label="${doChangeOrder_N}" onClick="doChangeOrder" disabled="${doChangeOrder_D}" visible="${doChangeOrder_V}"/-->
