...
<e:ui locale="${ses.countryCd}" lang="${ses.langCd}" dateFmt="${ses.dateFormat}">
    <script>
	    ...
	    function init() {
	        ...
	        grid.cellClickEvent(function(rowId, colId, value, iRow, iCol) {
                ...
                } else if(colId == 'NEXT_SIGN_USER_NM') {
                    if( grid.getCellValue(rowId, 'NEXT_SIGN_USER_ID') == '' ) return;
                    var param = {
                        callbackFunction : "",
                        USER_TYPE : 'B',  // C:운영사, B:고객사, S:공급사
                        USER_ID : grid.getCellValue(rowId, 'NEXT_SIGN_USER_ID'),
                        detailView : true
                    };
                    everPopup.openPopupByScreenId("BYM1_062", 680, 210, param);
                }
            ...
            if('${form.autoSearchFlag}' == 'Y') {
                ...
                store.setGrid([grid]);
                store.load(baseUrl + 'my02001_doSearch', function() {
	    ...
	    function doApproval() {
	    	...
	    	if(confirm("${msg.M0025 }")) {
	            ...
	            store.load(baseUrl + 'my02001_doApproval', function(){
    ...
    <e:window id="MY02_001" onReady="init" initData="${initData}" title="${fullScreenName}" breadCrumbs="${breadCrumb }">
		<e:inputHidden id="LOGIN_STATUS" name="LOGIN_STATUS" value="${loginStatus }"/>

		<e:searchPanel id="form" title="${form_CAPTION_N}" labelWidth="${labelWidth}" width="100%" columnCount="2" onEnter="doSearch" useTitleBar="false">
            <e:row>
