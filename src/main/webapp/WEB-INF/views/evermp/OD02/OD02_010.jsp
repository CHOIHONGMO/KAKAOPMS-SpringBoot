...
<e:ui locale="${ses.countryCd}" lang="${ses.langCd}" dateFmt="${ses.dateFormat}">
    <script>
        ...
        function init() {
            ...
            grid = EVF.C("grid");
            EVF.C('FLAG').setValue('1');
            <c:if test = "${form.type == 'B'}">
            EVF.C('FLAG').setValue('0');
            </c:if>
        ...
        function doSearch() {
            ...
            store.setGrid([grid]);
            store.load(baseUrl + "OD02_010/doSearch", function () {
           	...
           	for( var i in rowIds ) {
                ...
                if(cpoQty > 0) {
                    ...
                    if( Number(grid.getCellValue(rowIds[i], 'INV_QTY')) <= 0 ) {
                        return alert("${OD02_010_006}");
                    }

                    if( (bfInvQty + invQty) > cpoQty ) {
                        return alert("${OD02_010_011}");
                    }
                } else {
                    if( Number(grid.getCellValue(rowIds[i], 'INV_QTY')) >= 0 ) {
                        return alert("${OD02_010_012}");
                    }
                ...
                if( grid.getCellValue(rowIds[i], "DELY_TYPE") == '02'
                    ...
                    return alert("${OD02_010_031}");
                }
            }

            if(!confirm('${OD02_010_010}')) { return; }

            var store = new EVF.Store();
            store.setGrid([grid]);
            store.getGridData(grid, 'sel');
            store.load(baseUrl + 'OD02_010/doCreateInvoice', function() {
        ...
        function doDivideInvoice() {
            ...
            var rowIds = grid.getSelRowId();
           	for( var i in rowIds ) {
                ...
                //	return alert("${msg.M0008}");
               	//}
                //if (grid.getCellValue(rowIds[i], "IF_CPO_NO_SEQ") != "") {
        ...
        function onSearchPlant() {
            if( EVF.V("CUST_CD") == "" ) return alert("${OD02_010_001}");
            var param = {
    ...
    <e:window id="OD02_010" onReady="init" initData="${initData}" title="${fullScreenName}">
        <e:searchPanel id="form" title="${form_CAPTION_N}" labelWidth="${labelWidth}" labelAlign="${labelAlign}" columnCount="3" useTitleBar="false" onEnter="doSearch">
            ...
            <e:row>
                <e:label for="START_DATE">
                    <e:select id="START_DATE_COMBO" name="START_DATE_COMBO" width="99" disabled="false" readOnly="false" required="false" usePlaceHolder="false">
                        <e:option text="${OD02_010_029}" value="PO_DATE"/>
                        <e:option text="${OD02_010_020}" value="CPO_DATE"/>
                        <e:option text="${OD02_010_021}" value="HOPE_DUE_DATE"/>
                    </e:select>
                </e:label>
                <e:field>
            ...
            <e:row>
                ...
                <e:field>
                    ...
                    <e:inputHidden id="FLAG" name="FLAG"/>
                </e:field>
        ...
        <e:panel id="leftP1" height="fit" width="40%">
            <e:text style="font-weight: bold;">●&nbsp;${OD02_010_CAPTION1} &nbsp;:&nbsp; </e:text>
        ...
        <e:panel id="rightP1" height="fit" width="60%">
            <e:buttonBar align="right" width="100%">
                ...
                <e:button id="doDivideInvoice" name="doDivideInvoice" label="${doDivideInvoice_N}" onClick="doDivideInvoice" disabled="${doDivideInvoice_D}" visible="${doDivideInvoice_V}"/>
            </e:buttonBar>
        </e:panel>
