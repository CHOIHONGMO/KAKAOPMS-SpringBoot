...
<e:ui locale="${ses.countryCd}" lang="${ses.langCd}" dateFmt="${ses.dateFormat}">
	<script>
	    var baseUrl = "/evermp/buyer/rq/RQ0340P01";
	    var grid;
	    var gridV;
			    ...
			    for(var i in allgrid){
				    if(allgrid[i].RANK === '1'){
						...
						if(allgrid[i].QTA_YN =='0'){
							gridV.setCellReadOnly(i, "SLT_FLAG", true);
						}
					}
			    }
		    }
		...
		function doSettle() {
			...
			for (var i = 0; i < gridV.getRowCount(); i++) {
				...
				if(gridV.getCellValue(i,'SLT_FLAG') === '1' ) {
					...
					var sltRmk = gridV.getCellValue(i,'SLT_RMK');
					if(rank !== '1' && sltRmk === ''){
						return alert('${RQ0340P01_0002}'); //선택된 협력업체의 순위가 1위가 아닌 경우 선정사유를 필수로 등록해야 합니다.
					}
			...
			if(settleCou === 0) {
				return alert('${RQ0340P01_0003}'); //선정된 협력업체가 없습니다.
			}
			//단일업체인데 선정여부가 두명이상 Y일때 REURN;
			if(settleCou >1){
				return alert('${RQ0340P01_0008}');
			}
			if (!confirm("${RQ0340P01_0004}")) return; //선택된 협력업체로 업체선정 품의를 진행 하시겠습니까?
		...
		function doCancelRfq() {
			if (!confirm("${RQ0340P01_0005}")) return;
    		var store = new EVF.Store();
	    ...
	    function doApprovalList(){
		    if(EVF.isEmpty(EVF.V("APP_DOC_NUM")) || EVF.isEmpty(EVF.V("APP_DOC_CNT"))){ return alert("${RQ0340P01_0007}");}
		    var params = {
			    GATE_CD 			: ${ses.gateCd},
	...
	<e:window id="RQ0340P01" onReady="init" initData="${initData}" width="100%" height="100%" name="${fullScreenName}" title="${fullScreenName}">
        <e:searchPanel id="form" title="견적비교(총액별)" labelWidth="150" labelAlign="${labelAlign}" columnCount="2" onEnter="doSearch">
