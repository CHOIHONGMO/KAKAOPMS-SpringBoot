...
// 				return EVF.alert('${RQ0310P01_003}');
// 			}

// 			if(EVF.V('WH_NM_FLAG') === 'on' && EVF.V('PLANT_NM_FLAG') !== 'on'){
// 				return EVF.alert('${RQ0310P01_004}');
// 			}

			var paramData = [];
			$(':input').each(function(i, v){
				var vid = v.id;
				console.log(EVF.V(vid))
...
// 			setAutoCheck(data.CUST_CD, "PR_BUYER_NM");
		}
...
// 			$('#WH_NM_FLAG').attr("checked", false);
		...
		function selectWH(){
// 			if(EVF.V('PR_PLANT_NM') === '') {
// 				return EVF.alert('${RQ0310P01_002}');
// 			}
			var param = {
// 				PLANT_CD : EVF.V("PR_PLANT_CD"),
// 				PLANT_NM : EVF.V("PR_PLANT_NM"),
	...
	<e:window id="RQ0310P01" onReady="init" initData="${initData}" title="${fullScreenName}">

		<e:buttonBar id="buttonBar" align="right" width="100%" height="fit">
			<e:check id="selectAll" name="selectAll" onClick="doSelectAll" label="${RQ0310P01_000}"/>
...
<%-- 				<e:field> --%>
<%-- 					<e:text>&nbsp;</e:text> --%>
<%-- 					<e:check id="PR_BUYER_NM_FLAG" name="PR_BUYER_NM_FLAG" /> --%>
<%-- 					<e:text>&nbsp;</e:text> --%>
<%-- 				<e:search id="PR_BUYER_NM" name="PR_BUYER_NM" value="" width="${form_PR_BUYER_NM_W}" maxLength="${form_PR_BUYER_NM_M}" onIconClick="${form_PR_BUYER_NM_RO ? 'everCommon.blank' : 'selectBuyer'}" disabled="${form_PR_BUYER_NM_D}" readOnly="${form_PR_BUYER_NM_RO}" required="${form_PR_BUYER_NM_R}" /> --%>
...
<%-- 			<e:row> --%>
<%-- 				플랜트명 --%>
<%-- 				<e:label for="PR_PLANT_NM" title="${form_PR_PLANT_NM_N}"/> --%>
<%-- 				<e:field> --%>
<%-- 					<e:text>&nbsp;</e:text> --%>
<%-- 					<e:check id="PR_PLANT_NM_FLAG" name="PR_PLANT_NM_FLAG" /> --%>
<%-- 					<e:text>&nbsp;</e:text> --%>
<%-- 					<e:search id="PR_PLANT_NM" name="PR_PLANT_NM" value="" width="${form_PR_PLANT_NM_W}" maxLength="${form_PR_PLANT_NM_M}" onIconClick="${form_PR_PLANT_NM_RO ? 'everCommon.blank' : 'selectPlant'}" disabled="${form_PR_PLANT_NM_D}" readOnly="${form_PR_PLANT_NM_RO}" required="${form_PR_PLANT_NM_R}" /> --%>
<%-- 					<e:inputHidden id="PR_PLANT_CD" name="PR_PLANT_CD" /> --%>
