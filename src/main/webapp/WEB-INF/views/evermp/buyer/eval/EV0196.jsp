...
<link href="/css/kakao/ui2/sub.css" rel="stylesheet">
    ...
    function doSearch() {
        ...
        store.load(baseUrl + 'EV0196/doSearch', function() {
        	<%--// Map 형태의 Key 값을 받아서 --%>
				...
				$("#item_div div").remove();
        	   	...
        	   	if( evvu != null ) {
	        	   	...
	        	   	if( eval_score != null && eval_score != -1 ) {              <%--  기 점수가 존재하면 점수를 세팅. --%>
	        			EVF.C("EVAL_SCORE").setValue(eval_score);
	        	   	}
	        	   <%--
	        	   	if( att_file_num != null && att_file_num != "" ) {               기 점수가 존재하면 점수를 세팅.
	        		   EVF.C("ATT_FILE_NUM").setValue(att_file_num);
	        	   	}
	        	   	if( rmk != null && rmk != "" ) {               기 점수가 존재하면 점수를 세팅.
	        		   EVF.C("RMK").setValue(rmk);
	        	   	}
	        	    --%>
        	   	}
			            		...
			            		if( item_type2 == item_type3 && item_subject == item_subject3 ) {
		            				if(item_cnt == 1) {
		            					if(scale_type == "A") {
                                            ...
                                            html += "<div class=\"title\">" + item_subject + " - <label style='font-weight: normal;'>" + item_contents + "</label></div>";
			            			...
			            			if(scale_type == "A") { //선택형
				            			...
				            			html += "	<input type=\"radio\" id='item_" + item_num + item_id_sq + "' name=\"item_" + item_num + "\" method_cd='" + item_method_cd + "' id_sq='" + item_id_sq + "' score='" + item_id_score + "' weight='" + item_weight + "' " + str_checked + " " + readOnly + " class=\"custom-control-input\">";
                                        ...
                                        html += "	<label class=\"bullet\" for=\"quantity_1\">점수입력 :</label>";
                                        html += "	<input class=\"number\" type=\"number\" id='item_" + item_num + item_id_sq + "' name='item_" + item_num + "' method_cd='" + item_method_cd + "' id_sq='" + item_id_sq + "' score='" + item_id_score + "' weight='" + item_weight + "' value='" + item_id_score2 + "' maxLength='3' placeholder=\"0\" " + readOnly + ">";
                                        html += "	<span class=\"point\">배점 [" + item_id_score + "점]</span>";
                                        html += "</div>";
                                        html += "<div>";
                                        html += "	<label class=\"bullet\" for=\"text_4\">평가의견 : </label>";
                                        html += "	<input type=\"text\" id='remark_" + item_num + item_id_sq + "' name='remark_" + item_num + "' value='" + item_id_remark + "' maxLength='1000' " + readOnly + " >";
                                        html += "</div>";
    ...
    function doSave() {
		...
		*/
		...
		EVF.confirm("${msg.M8888 }", function () {
        	...
        	EVF.C("EVAL_SCORE").setValue(totalScore);


        	store.doFileUpload(function() {
	        	store.load(baseUrl + 'srm196_doSave', function () {
					if (opener) {
						opener.doSearch();
    ...
    <e:window id="EV0196" onReady="init" initData="${initData}" title="${fullScreenName }" breadCrumbs="${breadCrumb }">
        <e:searchPanel id="form" title="General Information" columnCount="2" labelWidth="${labelWidth }" onEnter="doSearch">
        	<e:row>
                <%--협력사명--%>
                <e:label for="VENDOR_CD" title="${form_VENDOR_CD_N}"/>
				<e:field>
				<e:inputText id="VENDOR_CD" style="ime-mode:inactive" name="VENDOR_CD" value="${form.VENDOR_CD}" width="40%" maxLength="${form_VENDOR_CD_M}" disabled="${form_VENDOR_CD_D}" readOnly="${form_VENDOR_CD_RO}" required="${form_VENDOR_CD_R}"/>
            ...
            <e:row>
				...
				<e:field>
				<e:inputText id="EV_USER_NM" style="${imeMode}" name="EV_USER_NM" value="${form.EV_USER_NM}" width="${form_EV_USER_NM_W}" maxLength="${form_EV_USER_NM_M}" disabled="${form_EV_USER_NM_D}" readOnly="${form_EV_USER_NM_RO}" required="${form_EV_USER_NM_R}"/>
				...
				<e:field>
				<e:inputNumber id="EVAL_SCORE" name="EVAL_SCORE" value="${form.EVAL_SCORE}"  width="${form_EVAL_SCORE_W}" maxValue="${form_EVAL_SCORE_M}" decimalPlace="${form_EVAL_SCORE_NF}" disabled="${form_EVAL_SCORE_D}" readOnly="${form_EVAL_SCORE_RO}" required="${form_EVAL_SCORE_R}" />
