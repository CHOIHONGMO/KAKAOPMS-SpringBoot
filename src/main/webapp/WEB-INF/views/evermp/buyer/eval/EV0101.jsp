...
<e:ui>
    ...
    function doSearchL() {
		...
		store.load(baseUrl + '/EV0101/doSearch', function() {
		});
        ...
        for (var i = kkk.length - 1; i > -1; i--) {
                ...
                if (addOk == "ok") {
            		rightGrid.addRow([{
        	 					 ...
        	 					 , 'EV_ITEM_CONTENTS'     : leftGrid.getCellValue(kkk[i], "EV_ITEM_CONTENTS")
    ...
    function getEvItemType() {
        ...
        store.load(baseUrl + '/EV0110/changeComboItemKindR', function() {
        	if (this.getParameter("itemTypeCode") != null) {
                EVF.C('EVAL_TYPE_CB').setOptions(this.getParameter("itemTypeCode"));
        	}
        });
    }
    ...
    <e:window id="EV0101" onReady="init" initData="${initData}" width="100%" height="100%" name="${fullScreenName}" title="${fullScreenName}">
	<e:panel id="pnl1" width="100%">
    	<e:searchPanel id="form" title="${msg.M9999}" labelWidth="${labelWidth}" width="100%" columnCount="2" onEnter="doSearchL">
        	<e:row>
			<e:label for="EVAL_KIND_CB" title="${form_EVAL_KIND_CB_N}"/>
			<e:field>
			<e:select id="EVAL_KIND_CB" name="EVAL_KIND_CB" value="${param.evalKind}" options="${evalKind }" width="${form_EVAL_KIND_CB_W}" disabled="true" readOnly="${form_EVAL_KIND_CB_RO}" required="${form_EVAL_KIND_CB_R}" placeHolder="" />
			<e:inputHidden id="EV_ITEM_KIND_CD_R" name="EV_ITEM_KIND_CD_R" value="${param.evalKind}"/>


			</e:field>
			<e:label for="EVAL_TYPE_CB" title="${form_EVAL_TYPE_CB_N}"/>
			<e:field>
			<e:select id="EVAL_TYPE_CB" name="EVAL_TYPE_CB" value="${form.EVAL_TYPE_CB}" options="${evalType }" width="${form_EVAL_TYPE_CB_W}" disabled="${form_EVAL_TYPE_CB_D}" readOnly="${form_EVAL_TYPE_CB_RO}" required="${form_EVAL_TYPE_CB_R}" placeHolder="" />
			</e:field>
            </e:row>
        	<e:row>
				<e:label for="EVAL_METHOD_CB" title="${form_EVAL_METHOD_CB_N}"/>
				<e:field>
				<e:select id="EVAL_METHOD_CB" name="EVAL_METHOD_CB" value="${form.EVAL_METHOD_CB}" options="${evalMethod }" width="${form_EVAL_METHOD_CB_W}" disabled="${form_EVAL_METHOD_CB_D}" readOnly="${form_EVAL_METHOD_CB_RO}" required="${form_EVAL_METHOD_CB_R}" placeHolder="" />
				</e:field>
