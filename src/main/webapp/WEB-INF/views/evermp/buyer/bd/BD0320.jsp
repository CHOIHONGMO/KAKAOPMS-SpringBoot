...
<e:ui locale="${ses.countryCd}" lang="${ses.langCd}" dateFmt="${ses.dateFormat}">
	<script>
	    var baseUrl = "/evermp/buyer/bd/BD0320";
	    var grid;
		var selRow;
	    ...
	    function init() {
		    ...
		    EVF.C('PROGRESS_CD').removeOption('2800');
	    ...
	    //2600: 작성중, 2650: 대기중, 2700: 진행중, 2750: 마감, 2850: 협상중
	    function fn_multiCombo(){
	    	if('${form.autoSearchFlag}' == 'Y') {
            	var chkName = "";
                $('.ui-multiselect-checkboxes li input').each(function (k, v) {
                    if(v.value == '2600' || v.value == '2650' || v.value == '2700' || v.value == '2750') {
                        chkName += v.title + ", ";
                        v.checked = true;
                    }
                });
		...
		function doTransferOpenUser(){
			...
			var param = {
				custCd           : "${ses.companyCd}",
				callBackFunction : "transferOpenUser",
			};
			everPopup.openCommonPopup(param, "SP0906");
		}

		function transferOpenUser(data){
			EVF.V('OPEN_USER_TRANSFER_NM', data.USER_NM);
			EVF.V('OPEN_USER_TRANSFER_ID', data.USER_ID);
		}
		...
		function doCtrlIdChg(){
			...
			if((EVF.V('CTRL_USER_TRANSFER_ID') == null || EVF.V('CTRL_USER_TRANSFER_ID') == '')
			  &&(EVF.V('OPEN_USER_TRANSFER_ID') == null || EVF.V('OPEN_USER_TRANSFER_ID') == '')){
				return alert("${BD0320_0003}");
			}
			...
			if(!confirm('${BD0320_0004}')) return; //이관 하시겠습니까?
		...
		function doModify() {
			...
			if (progressCd !== '2600' && progressCd !== '2650') {	// 2600: 입찰작성중, 2650: 입찰대기
				return EVF.alert("${msg.M0044}");
			} else {
				if (progressCd === '2600' && signStatus === 'P') {	// 2600: 입찰작성중, P: 결재중
					return EVF.alert("${msg.M0044}");
				}
			}
	    ...
	    function doChgStartDate() {
		    ...
		    var progressCd = grid.getCellValue(rowId,'PROGRESS_CD');
		    if (progressCd == '1400' || progressCd == '2600' || progressCd == '2900' || progressCd >= '2900') {
			    return alert("${msg.M0044}");
		    }
		    ...
		    everPopup.openPopupByScreenId('BD0320P01', 600, 440, param);
	    }
	    ...
	    function doChgEndDate() {
		    ...
		    if( submitVendorCnt > 0){ return alert('${BD0320_0001}') }
		...
		function doCloseRfq() {
			...
			var progressCd = grid.getCellValue(rowId,'PROGRESS_CD');
			if (progressCd != '2700'){ return alert("${msg.M0044}"); }
	    ...
	    function callback_doCloseRfqSave(text, rowId) {
		    ...
		    if (!confirm("${BD0320_0002}")) return;
		    if(text.trim() === '') { return alert('${BD0320_0005}'); }
	...
	<e:window id="BD0320" onReady="init" initData="${initData}" width="100%" height="100%" name="${fullScreenName}" title="${fullScreenName}">
			...
			<e:row>
				...
				<e:label for="DATE_FROM">
					<e:select id="TYPE" name="TYPE" value="" options="${typeOptions}" width="${form_TYPE_W}" disabled="${form_TYPE_D}" readOnly="${form_TYPE_RO}" required="${form_TYPE_R}" placeHolder="" usePlaceHolder="false">
						<e:option text="${BD0320_REG_DATE}" value="REG_DATE">${BD0320_REG_DATE}</e:option>
						<e:option text="${BD0320_RFX_TO_DATE}" value="RFX_TO_DATE">${BD0320_RFX_TO_DATE}</e:option>
						<e:option text="${BD0320_RFX_FROM_DATE}" value="RFX_FROM_DATE">${BD0320_RFX_FROM_DATE}</e:option>
					</e:select>
				</e:label>
				<e:field>
        ...
        <e:buttonBar align="right">
			...
			<e:panel>
				...
				<e:text>&nbsp;</e:text>
				<e:text style="font-weight: bold;">●&nbsp;${form_OPEN_USER_TRANSFER_NM_N } &nbsp;:&nbsp; </e:text>
				<e:search id="OPEN_USER_TRANSFER_NM" name="OPEN_USER_TRANSFER_NM" value="" width="${form_OPEN_USER_TRANSFER_NM_W}" maxLength="${form_OPEN_USER_TRANSFER_NM_M}" onIconClick="${form_OPEN_USER_TRANSFER_NM_RO ? 'everCommon.blank' : 'doTransferOpenUser'}" disabled="${form_OPEN_USER_TRANSFER_NM_D}" readOnly="${form_OPEN_USER_TRANSFER_NM_RO}" required="${form_OPEN_USER_TRANSFER_NM_R}" />
				<e:inputHidden id="OPEN_USER_TRANSFER_ID" name="OPEN_USER_TRANSFER_ID" />
				<e:text>&nbsp;</e:text>
