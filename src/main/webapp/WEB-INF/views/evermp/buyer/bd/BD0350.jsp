...
<e:ui locale="${ses.countryCd}" lang="${ses.langCd}" dateFmt="${ses.dateFormat}">
	<script>
	    var baseUrl = "/evermp/buyer/bd/BD0350";
	    var grid;
		var selRow;
	    ...
	    function init() {
		    ...
		    EVF.C('PROGRESS_CD').removeOption('1400');	// 유찰
		    EVF.C('PROGRESS_CD').removeOption('2600');	// 작성중
		    //EVF.C('PROGRESS_CD').removeOption('2650');	// 대기
		    EVF.C('PROGRESS_CD').removeOption('2900');	// 선정완료
		    EVF.C('PROGRESS_CD').removeOption('2950');	// 재입찰
		    ...
		    grid.cellClickEvent(function(rowId, colId, value, iRow, iCol) {
		    	...
		    	if(colId == "multiSelect") {
		           ...
		           everPopup.openPopupByScreenId('commonTextContents', 650, 350, param);
				    ...
				    everPopup.openPopupByScreenId('BD0350P01', 900, 600, param);
	    ...
	    function fn_multiCombo(){
	    	if('${form.autoSearchFlag}' == 'Y') {
                ...
                $('.ui-multiselect-checkboxes li input').each(function (k, v) {
                    if(v.value == '2650' || v.value == '2700' || v.value == '2750' || v.value == '2800' || v.value == '2850') {
                        chkName += v.title + ", ";
                        v.checked = true;
                    }
                });
		...
		function doModify() {
			...
			if(grid.getCellValue(rowId, 'CTRL_USER_ID') != "${ses.userId}" && '${ses.ctrlCd}'.indexOf('M100') < 0) {
	...
	<e:window id="BD0350" onReady="init" initData="${initData}" width="100%" height="100%" name="${fullScreenName}" title="${fullScreenName}">
			...
			<e:row>
				...
				<e:label for="DATE_FROM">
					<e:select id="TYPE" name="TYPE" value="" options="${typeOptions}" width="${form_TYPE_W}" disabled="${form_TYPE_D}" readOnly="${form_TYPE_RO}" required="${form_TYPE_R}" placeHolder="" usePlaceHolder="flase">
						<e:option text="${BD0350_REG_DATE}" value="REG_DATE">${BD0350_REG_DATE}</e:option>
						<e:option text="${BD0350_RFX_TO_DATE}" value="RFX_TO_DATE">${BD0350_RFX_TO_DATE}</e:option>
						<e:option text="${BD0350_RFX_FROM_DATE}" value="RFX_FROM_DATE">${BD0350_RFX_FROM_DATE}</e:option>
					</e:select>
				</e:label>
				<e:field>
        ...
        <e:buttonBar align="right">
			...
			<e:panel>
				...
				<e:button id="doModify" name="doModify" label="${doModify_N}" onClick="doModify" disabled="${doModify_D}" visible="${doModify_V}"/><%--<e:button id="doChgStartDate" name="doChgStartDate" label="${doChgStartDate_N}" onClick="doChgStartDate" disabled="${doChgStartDate_D}" visible="${doChgStartDate_V}"/>
