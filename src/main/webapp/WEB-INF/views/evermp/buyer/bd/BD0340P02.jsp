...
<e:ui locale="${ses.countryCd}" lang="${ses.langCd}" dateFmt="${ses.dateFormat}">
	<script>
	    var baseUrl = "/evermp/buyer/bd/BD0340P02";
	    var grid;
	    ...
	    function callback_setSltRmk(text, rowId) {
	    	...
	    	EVF.confirm('${BD0340P02_0008}', function(){
	    ...
	    function doSearch() {
	        ...
	        store.load(baseUrl + "/doSearch", function() {
		        setBgColor();
		        setSltResult();
	        /*<c:if test="${form.PROGRESS_CD == '2900' && form.SLT_FLAG == '0'}">
		...
		function doCancelRfq() {
			if (!confirm("${BD0340P02_0003}")) return;
    		var store = new EVF.Store();
		...
		function doSettle() {
			...
			if(grid.getRowCount() === 0) { return EVF.alert("${msg.M0004}"); }
	        ...
	        for (var i = 0; i < grid.getRowCount(); i++) {
	        	...
	        	for (var j = 0; j < grid.getRowCount(); j++) {
	        		...
	        		if(grid.getCellValue(j, 'SLT_FLAG') == "1" && grid.getCellValue(j, 'RANK') != "1" && grid.getCellValue(j, 'SLT_RMK') == ""){
						return alert('${BD0340P02_0006}'); //선택된 협력업체 품목의 순위가 1위가 아닌 경우 선정사유를 필수로 등록해야 합니다.
					}
	        	}

	        	if (countAward > 1) {
	        		alert('${BD0340P02_0004}'); //업체선정여부는 한건만 입력하시기 바랍니다.
	                return;
	            }
	        	if (countAward == 0 && grid.getCellValue(i, 'QTA_YN') !="N") {
	        		alert('${BD0340P02_0005}'); //품목별로 한 업체를 꼭 선정하셔야 합니다.
	                return;
	            }
	        }
			...
			if (!confirm("${BD0340P02_0002}")) return;
		...
		function doApprovalList(){
			if(EVF.isEmpty(EVF.V("APP_DOC_NUM")) || EVF.isEmpty(EVF.V("APP_DOC_CNT"))){ return alert("${BD0340P02_0007}");}
			var params = {
				GATE_CD : ${ses.gateCd},
	...
	<e:window id="BD0340P02" onReady="init" initData="${initData}" width="100%" height="100%" name="${fullScreenName}" title="${fullScreenName}">
        <e:searchPanel id="form" title="입찰비교(품목별)" labelWidth="150" labelAlign="${labelAlign}" columnCount="2" onEnter="doSearch">
		...
		<c:if test="${form.PROGRESS_CD == '2800' || (form.PROGRESS_CD == '2900' && form.SLT_FLAG == '0')}">
