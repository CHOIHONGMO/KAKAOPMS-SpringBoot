			...
			grid.cellClickEvent(function (rowId, colId, value) {
				...
				switch (colId) {
                    ...
                    case 'ETC_ATT_FILE_CNT':
                        if(value != '') {
                            var param = {
//                                havePermission: (contUserId == '${ses.userId}' || grantedAuthCd == '${ses.grantedAuthCd}') ? 'true' : 'false',
                            ...
                            everPopup.openPopupByScreenId('commonFileAttach', 650, 320, param);
                        }
                        break;
					default:
						return;
		...
		function doChangeContUser() {
			...
			for(var i = 0; i < rowIds.length; i++) {
				if (grid.getCellValue(rowIds[i], 'CONT_USER_ID') != '${ses.userId}') {
					return EVF.alert("${CT0360_0015}");
				}
			}

			EVF.confirm("${CT0360_0012 }", function () {
				var store = new EVF.Store();
				store.setGrid([grid]);
				store.getGridData(grid, 'sel');
				store.setParameter("CONT_USER_ID", EVF.V("NEW_CTRL_USER_ID"));
				store.load(baseUrl+'/CT0360/doChangeContUser', function() {
		...
		function doSearch() {
			...
			store.load(baseUrl + '/CT0360/doSearch', function () {
		...
		function doModCont() {
	            ...
	            for(var i in selRowId) {
	                if (grid.getCellValue(selRowId[i], "PROGRESS_CD") != "4300") {
	                    return EVF.alert("${CT0360_0027}");
	                }
	                if (grid.getCellValue(selRowId[i], "NEXT_CONT_CNT_FLAG") == "Y") {
	                    return EVF.alert("${CT0360_0031}");
	                }
		...
		function doSave() {
			...
			store.setGrid([grid]);
			store.getGridData(grid, 'sel');
			store.load(baseUrl+'/CT0360/doSave', function() {
				EVF.alert(this.getResponseMessage(), function() {
					doSearch();
				});
			});
	...
	<e:window id="CT0360" onReady="init" initData="${initData}" title="${fullScreenName }" breadCrumbs="${breadCrumb }">
