...
<e:ui locale="${ses.countryCd}" lang="${ses.langCd}" dateFmt="${ses.dateFormat}">
    <script>
            ...
            grid.cellClickEvent(function(rowId, colId, value) {
                ...
                if(colId === "multiSelect") {
                    ...
                    param = {
                        ...
                        PO_NO: grid.getCellValue(rowId, "PO_NO"),
                        CUST_CD: grid.getCellValue(rowId, "CUST_CD"),
                        detailView: true
                    };
                    everPopup.openPopupByScreenId("BOD1_111", 1100, 700, param);
        ...
        function doSearch() {
            ...
            store.setGrid([grid]);
            store.load(baseUrl + "bod1120Dosearch", function () {
		...
		function onSearchPlant() {
			if( EVF.V("CUST_CD") == "" ) return alert("${BOD1_120_007}");
			var param = {
		...
		function doBackCancel() {
            ...
            var rowIds = grid.getSelRowId();
                ...
                */
                if(grid.getCellValue(rowIds[i], 'SIGN_STATUS') != "N") {
                	return alert("${BOD1_120_002}");
                }
           	}

            if(!confirm("${BOD1_120_001}")) { return; }


            store.setGrid([grid]);
            store.getGridData(grid, 'sel');
            store.load(baseUrl + 'doBackCancel', function() {
                alert(this.getResponseMessage());
                doSearch();
            });
		}

		function doBackAgree() {
            var store = new EVF.Store();
            if (!store.validate()) { return; }
            if(!grid.isExistsSelRow()) { return alert("${msg.M0004}"); }
            ...
            if(!confirm("${BOD1_120_004}")) { return; }

            store.setGrid([grid]);
            store.getGridData(grid, 'sel');
            store.load(baseUrl + 'doBackAgree', function() {
                alert(this.getResponseMessage());
                doSearch();
            });
		}
    ...
    <e:window id="BOD1_120" onReady="init" initData="${initData}" title="${fullScreenName}">
        ...
        <e:buttonBar width="100%" align="right">
			...
			<e:button id="doBackCancel" name="doBackCancel" label="${doBackCancel_N}" onClick="doBackCancel" disabled="${doBackCancel_D}" visible="${doBackCancel_V}"/>

			<c:if test="${ses.userType == 'C' }">
				<e:button id="doBackAgree" name="doBackAgree" label="${doBackAgree_N}" onClick="doBackAgree" disabled="${doBackAgree_D}" visible="${doBackAgree_V}"/>
			</c:if>
        </e:buttonBar>
