...
<e:ui locale="${ses.countryCd}" lang="${ses.langCd}" dateFmt="${ses.dateFormat}">
    ...
    <script>
        ...
        function init() {
            ...
            grid.cellClickEvent(function(rowId, colId, value) {
				...
				if (colId === "MGRNO") {
                        ...
                        store.load("/evermp/TX01/tx01020_detail", function () {
        ...
        function doSearch() {
            ...
            store.setGrid([grid]);
            store.load(baseUrl + "tx02030_doSearch", function () {
        ...
        function doAccV() {
            ...
            for( var i in rowIds ) {
                if (grid.getCellValue(rowIds[i], "ACCOUNT_CHECK_YN") == "Y") {
                    return alert("${TX02_030_006}"); // 이미 회계검증된 계산서 입니다.
                }
            }

            if (!confirm("${TX02_030_001 }")) return;

            var store = new EVF.Store();
            store.setGrid([grid]);
            store.getGridData(grid, "sel");
            store.load("/evermp/TX01/tx01020_doAccV", function () {
                doSearch();
        ...
        function doAccV_Cancel() {
            ...
            for( var i in rowIds ) {
                if (grid.getCellValue(rowIds[i], "ACCOUNT_CHECK_YN") == "N") {
                    return alert("${TX02_030_012}"); // 회계검증을 진행 후 회계검증취소를 하여 주시기 바랍니다.
                }

                if (grid.getCellValue(rowIds[i], "ACCOUNT_CONFIRM_YN") == "Y") {
                    return alert("${TX02_030_007}");
                }
            }

            if (!confirm("${TX02_030_002 }")) return;

            var store = new EVF.Store();
            store.setGrid([grid]);
            store.getGridData(grid, "sel");
            store.load("/evermp/TX01/tx01020_doAccV_Cancel", function () {
                doSearch();
                return alert('${msg.M0001}');
            });
        }
        ...
        function doAccC() {
            ...
            for( var i in rowIds ) {
                if (grid.getCellValue(rowIds[i], "ACCOUNT_CONFIRM_YN") == "Y") {
                    return alert("${TX02_030_007}"); // 이미 회계확정된 계산서 입니다.
                }

                if (grid.getCellValue(rowIds[i], "ACCOUNT_CHECK_YN") == "N") {
                    return alert("${TX02_030_014}");
                }
            }

            if (!confirm("${TX02_030_003 }")) return;

            var store = new EVF.Store();
            store.setGrid([grid]);
            store.getGridData(grid, "sel");
            store.load("/evermp/TX01/tx01020_doAccC", function () {
                doSearch();
                return alert('${msg.M0001}');
            });
        }
        ...
        function doAccC_Cancel() {
            ...
            for( var i in rowIds ) {
                if (grid.getCellValue(rowIds[i], "ACCOUNT_CONFIRM_YN") == "N") {
                    return alert("${TX02_030_011}"); // 회계확정을 진행 후 회계확정취소를 하여 주시기 바랍니다.
                }
            }

            if (!confirm("${TX02_030_004 }")) return;

            var store = new EVF.Store();
            store.setGrid([grid]);
            store.getGridData(grid, "sel");
            store.load("/evermp/TX01/tx01020_doAccC_Cancel", function () {
                doSearch();
                return alert('${msg.M0001}');
            });
        }
        ...
        function doAutoDocExe() {
            ...
            for( var i in rowIds ) {
                if (grid.getCellValue(rowIds[i], "STATUS") == "S") {
                    return alert("${TX02_030_008}"); // 이미 전표이관된 계산서 입니다.
                }

            }

            if(confirmYN == "Y") {
                if (!confirm("${TX02_030_005 }")) return;
            } else {
                if (!confirm("${TX02_030_016 }")) return;
            }

            store.setGrid([grid]);
            store.getGridData(grid, "sel");
            store.load("/evermp/TX01/tx01020_doAutoDocExe", function () {
                doSearch();
                return alert('${TX02_030_018}');
            });
        }
        function doSalesClose() {
            ...
            for( var i in rowIds ) {
                if (grid.getCellValue(rowIds[i], "ACCOUNT_CLOSE_YN") == "Y") {
                    return alert("${TX02_030_013}"); // 이미 월매출마감종료된 계산서 입니다.
                }
            }
*/
            if (!confirm(everString.getMessage("${TX02_030_009 }", EVF.V("CLOSE_YEAR") + "년 " + EVF.V("CLOSE_MONTH") + "월") +
                    "\n" + everString.getMessage("${TX02_030_010 }", EVF.V("CLOSE_YEAR") + "년 " + EVF.V("CLOSE_MONTH") + "월"))) return;

            var store = new EVF.Store();
            store.setGrid([grid]);
            store.getGridData(grid, "sel");
            store.setParameter("TYPE", "P");
            store.load("/evermp/TX01/tx01020_doSalesClose", function () {
                doSearch();
                chkClose();
                return alert('${msg.M0001}');
            });
        ...
        function doSalesCloseCancel() {
            var store = new EVF.Store();
            store.setParameter("TYPE", "P");
            store.load("/evermp/TX01/doSalesCloseCancel", function () {
                doSearch();
                chkClose();
                return alert('${msg.M0001}');
            });
        ...
        function chkClose() {
	            var store = new EVF.Store();
	            store.setParameter("TYPE", "P");
	            store.load("/evermp/TX01/chkClose", function () {
    ...
    <e:window id="TX02_030" onReady="init" initData="${initData}" title="${fullScreenName}">
        <%-- 매출 : S, 매입 : P --%>
        <e:inputHidden id="C_FLAG" name="C_FLAG" value="P" />
        ...
        <e:searchPanel id="form" title="${form_CAPTION_N}" labelWidth="${labelWidth}" labelAlign="${labelAlign}" columnCount="3" useTitleBar="false" onEnter="doSearch">
            <e:row>
                ...
                <e:field>
					...
					<e:inputHidden id="TYPE" name="TYPE" value="AP"/>
                </e:field>
