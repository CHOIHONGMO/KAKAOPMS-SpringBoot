...
//                    EVF.V("USER_TYPE","C");
        ...
        function doSave() {
            ...
            if(EVF.V("ID_CHECK")==""){
                return alert("${BS03_006_1_002}");
            }

/*
            if(EVF.V("TIER_CD_P")!=""){
                if(EVF.V("TIER_CD_P")!=EVF.V("TIER_CD")){
                    if(EVF.V("AMEND_REASON")==""){
                        alert("${BS01_002_001 }");
                        EVF.C('TIER_CD').setFocus();
                        return;
                    }
                }
            }
            ...
            if(!confirm(btnMsg)) { return; }
                ...
                store.load(baseUrl + 'bs03006_doSave', function () {
                    alert(this.getResponseMessage());
                        ...
                        window.close();
                        //window.location.href = '/evermp/BS03/BS0302/BS03_006_1/view.so?' + $.param(param);
        	...
        	if(EVF.C("USER_ID").getValue()==""){
               ...
               store.load(baseUrl + 'bs01002_doCheckUserId', function() {
                   if(this.getParameter("POSSIBLE_FLAG") == "N") {
                       EVF.V("ID_CHECK","");
                       return alert("${BS03_006_1_006}");
                   }else{
                       EVF.V("ID_CHECK","Y");
                       alert("${BS03_006_1_001}");
...
*/
            ...
            if(SamePass_0 > 0) {
                alert("${BS03_006_1_022}");
                return false;
            }
            if(SamePass_1 > 1 || SamePass_2 > 1 ) {
                alert("${BS03_006_1_023}");
                return false;
            }
        ...
        function selectVendor_I(dataJsonArray) {
            ...
            EVF.C('PLANT_CD').setValue(dataJsonArray.VENDOR_CD);
        ...
        function selectCust_I(dataJsonArray) {
            ...
            EVF.C('IRS_NO').setValue(dataJsonArray.IRS_NO);
            EVF.V("DEPT_CD","");
            EVF.V("DEPT_NM","");
            EVF.V("BUDGET_DEPT_CD","");
            EVF.V("BUDGET_DEPT_NM","");
            if(dataJsonArray.APPROVE_USE_FLAG == '1')
            {
                EVF.V("MAIL_FLAG","1");
                EVF.C("MAIL_FLAG").setReadOnly(true);
            }
        }
        ...
        function deptCdSearch_I() {
            ...
            if (EVF.V("USER_TYPE") == "") {
                alert("${BS03_006_1_0002}");
                return;
            }

            if (EVF.V("COMPANY_CD") == "") {
                alert("${BS03_006_1_0005}");
                return;
            }
        ...
        function UserTypeSearchChange(){
            ...
            if(EVF.V("USER_TYPE")=="S"){
                ...
                EVF.C('DEPT_NM').setRequired(false);
                EVF.C('DEPT_CD').setRequired(true);
                ...
                EVF.C('USER_AUTO_PO_FLAG').setRequired(true);
                EVF.C('APCH_FLAG').setRequired(true);
    ...
    <e:window id="BS03_006_1" onReady="init" initData="${initData}" title="${fullScreenName}" breadCrumbs="${breadCrumb }">
            <e:buttonBar id="buttonTopBottom" align="right" width="100%" title="${BS03_006_1_CAPTION1 }">
        ...
        <e:searchPanel id="form1" title="${form_CAPTION_N }" labelWidth="${labelWidth}" width="100%" columnCount="2" useTitleBar="false">
            <e:row>
                ...
                <e:inputHidden id="USER_TYPE" name="USER_TYPE" value="S" />

                <e:inputHidden id="IRS_NO" name="IRS_NO" value="${formData.IRS_NO }" />
                <e:inputHidden id="DEPT_CD" name="DEPT_CD" value="${formData.DEPT_CD }" />
                <e:inputHidden id="DEPT_NM" name="DEPT_NM" value="${formData.DEPT_NM }" />
                <e:inputHidden id="PLANT_CD" name="PLANT_CD" value="${formData.PLANT_CD }" />
                <e:label for="COMPANY_CD" title="${form_COMPANY_CD_N}"></e:label>
                <e:field colSpan="3">
                    <e:search id="COMPANY_CD" name="COMPANY_CD" value="${formData.COMPANY_CD }" width="20%" maxLength="${form_COMPANY_CD_M}" onIconClick="companySearch_I" disabled="${form_COMPANY_CD_D}" readOnly="${form_COMPANY_CD_RO}" required="${form_COMPANY_CD_R}" />
                    <e:inputText id="COMPANY_NM" style='ime-mode:inactive' name="COMPANY_NM" value="${formData.COMPANY_NM}" width="50%" maxLength="${form_COMPANY_NM_M}" disabled="${form_COMPANY_NM_D}" readOnly="true" required="${form_COMPANY_NM_R}" />
                </e:field>
            </e:row>
        </e:searchPanel>
        ...
        <e:searchPanel id="form2" title="${form_CAPTION_N }" labelWidth="${labelWidth}" width="100%" columnCount="2" useTitleBar="false">
            <e:panel id="Panel2" height="fit" width="30%">
                <e:title title="${BS03_006_1_CAPTION2 }" depth="1"/>
            </e:panel>
            <e:row>
            ...
            <e:row>
                ...
                <e:field>
                    <e:inputText id="TEL_NUM" name="TEL_NUM"  placeHolder="${BS03_006_1_INPUT_T2 }" value="${formData.TEL_NUM}" width="100%" maxLength="${form_TEL_NUM_M}" disabled="${form_TEL_NUM_D}" readOnly="${form_TEL_NUM_RO}" required="${form_TEL_NUM_R}" onChange="checkTelNo" data="TEL_NUM" />
                </e:field>
                <e:label for="FAX_NUM" title="${form_FAX_NUM_N}"></e:label>
                <e:field>
                    <e:inputText id="FAX_NUM" name="FAX_NUM" placeHolder="${BS03_006_1_INPUT_T2 }" value="${formData.FAX_NUM}" width="100%" maxLength="${form_FAX_NUM_M}" disabled="${form_FAX_NUM_D}" readOnly="${form_FAX_NUM_RO}" required="${form_FAX_NUM_R}" onChange="checkTelNo" data="FAX_NUM" />
                </e:field>
            </e:row>
            <e:row>
                <e:label for="CELL_NUM" title="${form_CELL_NUM_N}"></e:label>
                <e:field>
                    <e:inputText id="CELL_NUM" name="CELL_NUM"  placeHolder="${BS03_006_1_INPUT_T2 }" value="${formData.CELL_NUM}" width="100%" maxLength="${form_CELL_NUM_M}" disabled="${form_CELL_NUM_D}" readOnly="${form_CELL_NUM_RO}" required="${form_CELL_NUM_R}" onChange="checkCellNo" data="CELL_NUM" />
                </e:field>
        ...
        <div id="CustCsdm" style="display: block;">
            <e:searchPanel id="custform5" title="${form_CAPTION_N }" labelWidth="${labelWidth}" width="100%" columnCount="2" useTitleBar="false">
                <e:panel id="custanel5" height="fit" width="30%">
                    <e:title title="${BS03_006_1_CAPTION6 }" depth="1"/>
                </e:panel>
                <e:row>
                    <e:label for="CSDM_SEQ" title="${form_CSDM_SEQ_N}"></e:label>
                    <e:field>
                        <e:search id="CSDM_SEQ" name="CSDM_SEQ" value="${formData.CSDM_SEQ }" width="30%" maxLength="${form_CSDM_SEQ_M}" onIconClick="csdmCdSearch" disabled="${form_CSDM_SEQ_D}" readOnly="${form_CSDM_SEQ_RO}" required="${form_CSDM_SEQ_R}" />
        ...
        <div id="CustCubl" style="display: block;">
            <e:searchPanel id="custform6" title="${form_CAPTION_N }" labelWidth="${labelWidth}" width="100%" columnCount="2" useTitleBar="false">
                <e:panel id="custanel6" height="fit" width="30%">
                    <e:title title="${BS03_006_1_CAPTION7 }" depth="1"/>
                </e:panel>
                <e:row>
                    <e:label for="CUBL_SQ" title="${form_CUBL_SQ_N}"></e:label>
                    <e:field>
                        <e:search id="CUBL_SQ" name="CUBL_SQ" value="${formData.CUBL_SQ }" width="30%" maxLength="${form_CUBL_SQ_M}" onIconClick="cublCdSearch" disabled="${form_CUBL_SQ_D}" readOnly="${form_CUBL_SQ_RO}" required="${form_CUBL_SQ_R}" />
        ...
        <div id="CustDiv" style="display: block;">
            <e:searchPanel id="custform2" title="${form_CAPTION_N }" labelWidth="${labelWidth}" width="100%" columnCount="2" useTitleBar="false">
                <e:panel id="custanel2" height="fit" width="30%">
                    <e:title title="${BS03_006_1_CAPTION5 }" depth="1"/>
                </e:panel>
                <e:row>
                ...
                <e:row>
                    ...
                    <e:field>
                        <e:search id="CHIEF_USER_NM" name="CHIEF_USER_NM" value="${formData.CHIEF_USER_NM}" width="${form_CHIEF_USER_NM_W}" maxLength="${form_CHIEF_USER_NM_M}" onIconClick="doUserSearch" disabled="${form_CHIEF_USER_NM_D}" readOnly="${form_CHIEF_USER_NM_RO}" required="${form_CHIEF_USER_NM_R}" onChange="clearUser" />
                    ...
                    <e:field>
                        <e:select id="USER_AUTO_PO_FLAG" name="USER_AUTO_PO_FLAG" value="${formData.USER_AUTO_PO_FLAG }" options="${userAutoPoFlagOptions }" width="${form_USER_AUTO_PO_FLAG_W}" disabled="${form_USER_AUTO_PO_FLAG_D}" readOnly="${form_USER_AUTO_PO_FLAG_RO}" required="${form_USER_AUTO_PO_FLAG_R}" placeHolder="" usePlaceHolder="false"/>
                    </e:field>
                ...
                <e:row>
                    ...
                    <e:label for="APROVAL_USER_FLAG" title="${form_APROVAL_USER_FLAG_N}" />
                    <e:field>
                        <e:select id="APROVAL_USER_FLAG" name="APROVAL_USER_FLAG" value="${formData.APROVAL_USER_FLAG }" options="${aprovalUserFlagOptions }" width="${form_APROVAL_USER_FLAG_W}" disabled="${form_APROVAL_USER_FLAG_D}" readOnly="${form_APROVAL_USER_FLAG_RO}" required="${form_APROVAL_USER_FLAG_R}" placeHolder="" usePlaceHolder="false"/>
                    </e:field>
                    <e:label for="APCH_FLAG" title="${form_APCH_FLAG_N}" />
                    <e:field>
                        <e:select id="APCH_FLAG" name="APCH_FLAG" value="${formData.APCH_FLAG }" options="${apchFlagOptions }" width="${form_APCH_FLAG_W}" disabled="${form_APCH_FLAG_D}" readOnly="${form_APCH_FLAG_RO}" required="${form_APCH_FLAG_R}" placeHolder="" usePlaceHolder="false"/>
                    </e:field>
                </e:row>
            </e:searchPanel>
        </div>
