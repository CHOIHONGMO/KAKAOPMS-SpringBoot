...
<e:ui locale="${ses.countryCd}" lang="${ses.langCd}" dateFmt="${ses.dateFormat}">
    <script>
        ...
        function doSearch(){
            ...
            store.setGrid([grid]);
            store.load(baseUrl + "po0610_doSearch", function () {
        ...
        function doConfirm(){
         ...
         if(selRowId == '' || selRowId == null ){
           ...
           for(var i in selRowId) {
			   ...
			   if (itemStatus == '20' || itemStatus == '30') {  //ITEM_STATUS가 [삭제] [단종] 시 주문안됨
				   return alert("${PO0610_0005}")
				   }

			   var PO_QTY = grid.getCellValue(selRowId[i], 'PO_QTY');
        	   if(PO_QTY == '0' ){
        		   return alert('${PO0610_0002}');
        		   }
        	   var addParam={
                     	  ...
                     	  ,'MAKER_NM' 		: grid.getCellValue(selRowId[i], 'MAKER_NM')
                       	  ...
                       	  ,'MOQ_QTY'  		: grid.getCellValue(selRowId[i], 'MOQ_QTY')
                       	  ,'CONT_UNIT_PRICE': grid.getCellValue(selRowId[i], 'CONT_UNIT_PRICE')
                       	  ,'RV_QTY'   		: grid.getCellValue(selRowId[i], 'RV_QTY')
                       	  ,'PO_ITEM_AMT' 	: grid.getCellValue(selRowId[i], 'PO_QTY') *  grid.getCellValue(selRowId[i], 'CONT_UNIT_PRICE')
                          ,'TAX_CD'   		: grid.getCellValue(selRowId[i], 'TAX_CD')
                          ,'LEAD_TIME'   	: grid.getCellValue(selRowId[i], 'LEAD_TIME')
                          ,'DELY_TYPE'   	: grid.getCellValue(selRowId[i], 'DELY_TYPE')
                          ,'CUST_ITEM_CD'   : grid.getCellValue(selRowId[i], 'CUST_ITEM_CD')
                          ,'SG_CTRL_USER_ID': grid.getCellValue(selRowId[i], 'SG_CTRL_USER_ID')
                          ,'SIGN_STATUS'	: grid.getCellValue(selRowId[i], 'SIGN_STATUS')
                          ,'CONT_NO'		: grid.getCellValue(selRowId[i], 'CONT_NO')
                          ,'CONT_SEQ'		: grid.getCellValue(selRowId[i], 'CONT_SEQ')
                          ,'UNIT_CD'		: grid.getCellValue(selRowId[i], 'UNIT_CD')
                          ,'ITEM_STATUS'	: grid.getCellValue(selRowId[i], 'ITEM_STATUS')
                       	  , rowId    	    : rowId
                 		 }
         ...
         var param ={
               ...
               ,'CUST_NM' : "${ses.companyNm}"
           }
            everPopup.openPopupByScreenId("SPO0601", 1100, 700, param);
          }
        }

    </script>

    <e:window id="PO0610" onReady="init" initData="${initData}" title="${fullScreenName}" breadCrumbs="${breadCrumb }">
