...
<e:ui locale="${ses.countryCd}" lang="${ses.langCd}" dateFmt="${ses.dateFormat}">
    <script type="text/javascript">
		...
		function init() {
			...
			var poGubun = '${form.PO_GUBUN}';

			if(poGubun=='2') {
				EVF.C("JOSIK_NAME1").setDisabled(true);
				EVF.C("JOSIK_NAME2").setDisabled(true);
			}
        ...
        function doSearch() {
        	...
        	store.setGrid([grid]);
            store.load(baseUrl + 'poList02', function() {
            });
        }
    ...
    <e:window id="POLIST02" onReady="init" initData="${initData}" title="${fullScreenName}" breadCrumbs="${breadCrumb }">
        <e:searchPanel id="form" title="${msg.M9999 }" columnCount="2" labelWidth="${labelWidth }" onEnter="doSearch" useTitleBar="false">
            ...
            <e:row>
				...
				<e:field>
				<e:inputText id="JOSIK_NAME1" name="JOSIK_NAME1" value="${form.COMPANY_NAME}" width="${form_JOSIK_NAME1_W}" maxLength="${form_JOSIK_NAME1_M}" disabled="${form_JOSIK_NAME1_D}" readOnly="${form_JOSIK_NAME1_RO}" required="${form_JOSIK_NAME1_R}" />
				</e:field>
				<e:label for="JOSIK_NAME2" title="${form_JOSIK_NAME2_N}" />
				<e:field>
				<e:inputText id="JOSIK_NAME2" name="JOSIK_NAME2" value="${form.DIVISION_NAME}" width="${form_JOSIK_NAME2_W}" maxLength="${form_JOSIK_NAME2_M}" disabled="${form_JOSIK_NAME2_D}" readOnly="${form_JOSIK_NAME2_RO}" required="${form_JOSIK_NAME2_R}" />
				</e:field>
