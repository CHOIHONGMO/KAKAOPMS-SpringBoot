...
<e:ui locale="${ses.countryCd}" lang="${ses.langCd}" dateFmt="${ses.dateFormat}">
    ...
    function getCloseDate() {
        ...
        store.load(baseUrl + "BSOA_031/getCloseDate", function() {
    ...
    function doSave() {
        ...
        everDate.diffWithServerDate(fromDate, function(status, message) {
            if (status === '-1') {
                alert('${BSOA_031_CLOSE_DATE}');

            } else {
                if (!confirm("${msg.M0021}")) return;

                var store = new EVF.Store();
                store.load(baseUrl + "BSOA_031/doSave", function() {
    ...
    <e:window id="BSOA_031" onReady="init" initData="${initData}" title="${fullScreenName}" margin="0 5px 0 5px">
        ...
        <e:searchPanel id="form" title="${form_CAPTION_N }" columnCount="1" labelWidth="135">
            <e:row>
            ...
            <e:row>
				...
				<e:field>
				<e:inputText id="EXTEND_RMK" style="${imeMode}" name="EXTEND_RMK" value="${form.EXTEND_RMK}" width="100%" maxLength="${form_EXTEND_RMK_M}" disabled="${form_EXTEND_RMK_D}" readOnly="${form_EXTEND_RMK_RO}" required="${form_EXTEND_RMK_R}"/>
				</e:field>
            </e:row>
        </e:searchPanel>
