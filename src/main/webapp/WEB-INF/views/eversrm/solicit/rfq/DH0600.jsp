...
<e:ui locale="${ses.countryCd}" lang="${ses.langCd}" dateFmt="${ses.dateFormat}">
    ...
    function doSearch() {
        ...
        store.load(baseUrl + "DH0600/doSearchVendor", function() {
    ...
    function doSearchItem() {
        ...
        store.setGrid([gridI]);
        store.load(baseUrl + "DH0600/doSearchItem", function() {
        });
    }
    ...
    function doSave() {
		...
		if (signStatus == 'P') { //결재요청일 경우
			...
			store.doFileUpload(function() {
	        	store.load(baseUrl + "/DH0600/doSave", function() {
			...
			store.doFileUpload(function() {
	        	store.load(baseUrl + "/DH0600/doSave", function() {
		            ...
		            alert(this.getResponseMessage());
		            opener.doSearch();
		            location.href='/eversrm/solicit/rfq/DH0600/view.so?SCREEN_ID=DH0600&popupFlag=true&itemList=false&EXEC_NUM='+this.getParameter("EXEC_NUM");
		        });
            });
		}
	}
	  	...
	  	store.doFileUpload(function() {
		   	store.load(baseUrl + "/doSave", function() {
		        ...
		        //alert(this.getParameter("EXEC_NUM"));
		            location.href='/eversrm/solicit/rfq/DH0600/view.so?SCREEN_ID=DH0600&popupFlag=true&EXEC_NUM='+this.getParameter("EXEC_NUM");
			    //if (popupFlag) {
				//	opener.doSearch();
			    //} else {
			    //}
    ...
    function doDelete() {
        ...
        store.setGrid([gridV]);
    	store.getGridData(gridV, 'all');
    	store.load(baseUrl + 'DH0600/doDelete', function(){
        	EVF.getComponent('TRANSACTION_FLAG').setValue('Y');
    ...
    function doSearchGwDocData() {
        ...
        store.setGrid([gwdoc]);
        store.load(baseUrl + "DH0600/doSearchGwDocData", function () {
        	gwdoc.checkAll(true);
  ...
  function doApply3() {
    ...
    <e:window id="DH0600" onReady="init" initData="${initData}" title="${fullScreenName}">
        ...
        <e:searchPanel id="form" title="${form_CAPTION_N}" labelWidth="${labelWidth}" labelAlign="${labelAlign}" columnCount="2">
            ...
            <e:field>
                <e:search id='CTRL_USER_NM' name="CTRL_USER_NM" value='${empty form.CTRL_USER_NM ? ses.userNm : form.CTRL_USER_NM}' width='${inputTextWidth }' required='${form_CTRL_USER_NM_R }' readOnly='true' disabled='${form_CTRL_USER_NM_D }' visible='${form_CTRL_USER_NM_V }' maxLength="${form_CTRL_USER_NM_M}" onIconClick="doSearchBuyer" />
