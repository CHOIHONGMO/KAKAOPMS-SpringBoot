...
<e:ui locale="${ses.countryCd}" lang="${ses.langCd}" dateFmt="${ses.dateFormat}">
    <script type="text/javascript">
        ...
        var baseUrl = "/eversrm/po/deliveryMgt/BPOI_010"
                , grid;
        var flag = 0;
        ...
        function doPrint(){
            ...
            */
            //var url = "/eversrm/po/deliveryMgt/BPOI_010_Report/view"
        ...
        QUnit.test( "TestSquare", function( assert ) {
            var result = sum(10);
            assert.equal( result, "10", "sum(10) should be 10." );
        });
    </script>
    <e:window id="BPOI_010" onReady="init" initData="${initData}" title="${fullScreenName}" breadCrumbs="${breadCrumb }">
        <e:searchPanel id="form" title="${msg.M9999}" columnCount="3" labelWidth="${labelWidth}" useTitleBar="false" onEnter="doSearch">
            <e:row>
                <e:label for="">
                    <e:select id="COMBO_DATE" name="COMBO_DATE" usePlaceHolder="false" width="100" required="${form_COMBO_DATE_R}" disabled="${form_COMBO_DATE_D }" readOnly="${form_COMBO_DATE_RO}">
                        <e:option text="${BPOI_010_INV_DATE}" value="INV_DATE">${BPOI_010_INV_DATE}</e:option>
                        <e:option text="${BPOI_010_DUE_DATE}" value="DUE_DATE">${BPOI_010_DUE_DATE}</e:option>
                        <e:option text="${BPOI_010_PO_DATE}" value="PO_DATE">${BPOI_010_PO_DATE}</e:option>
                    </e:select>
                </e:label>
                <e:field>
            ...
            <e:row>
                ...
                <e:field>
                    <e:search id="VENDOR_NM" style="${imeMode}" name="VENDOR_NM" value="" width="100%" maxLength="${form_VENDOR_NM_M}" onIconClick="${form_VENDOR_NM_RO ? 'everCommon.blank' : ''}" disabled="${form_VENDOR_NM_D}" readOnly="${form_VENDOR_NM_RO}" required="${form_VENDOR_NM_R}" />
                </e:field>
            </e:row>
            <e:row>
