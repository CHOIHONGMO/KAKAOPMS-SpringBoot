...
<e:ui locale="${ses.countryCd}" lang="${ses.langCd}" dateFmt="${ses.dateFormat}">
    <script type="text/javascript">
        ...
        function init() {
        	$("#P_PASSWORD").attr("autocomplete", 'new-password');
			if(EVF.V("PW_RESET_TYPE")=='Y'){
				EVF.C('PW_RESET_TYPE').setRequired(true);
        ...
        function doSave() {
            ...
            if(EVF.isNotEmpty(presentPass)) {
                if (newPassCheck1.trim() == '' || newPassCheck2.trim() == '') {
                    pwValid();
                    return alert("${MSI_080_003}");
                }

                if(newPassCheck1 != newPassCheck2) {
                    pwValid();
                    return alert('${MSI_080_004}');
                }
            }
            ...
            store.load(baseUrl + '/saveUser', function () {
    ...
    <e:window id="window" onReady="init" initData="${initData}" title="${fullScreenName}" breadCrumbs="${breadCrumb }">
    	...
    	<e:buttonBar width="100%" align="right" title="${MSI_080_TITLE1}">
        ...
        <e:searchPanel id="sp" title="${form_CAPTION_N}" labelWidth="${labelWidth}" useTitleBar="false" width="100%" columnCount="2">
			<e:inputHidden id="PW_RESET_TYPE" name="PW_RESET_TYPE" value="${form.PW_RESET_TYPE}" />
            <e:row>
