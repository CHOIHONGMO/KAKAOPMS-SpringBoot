...
<e:ui locale="${ses.countryCd}" lang="${ses.langCd}" dateFmt="${ses.dateFormat}">
    ...
    function init() {
        ...
        gridI.cellChangeEvent(function (rowId, celName, iRow, iCol, newValue, oldValue) {
			if (celName == "FINAL_PRC") {
            	if (Number(newValue) < 0) {
                	alert('${WM0850_0006}');
                	gridI.setCellValue(rowId, celName, oldValue);
                	return;
            	}
                gridI.setCellValue(rowId, "CHA_PRC", (gridI.getCellValue(rowId, "FINAL_PRC") - gridI.getCellValue(rowId, "GAUNIT_PRC")));
            }
        });
    ...
    function doSearch() {
        ...
        store.load(baseUrl + "DH0550/doSearchVendor", function() {
    ...
    function doSearchItem() {
        ...
        store.setGrid([gridI]);
        store.load(baseUrl + "DH0550/doSearchItem", function() {
        });
    }
    ...
    function doSave() {
		...
		if (signStatus == 'P') { //결재요청일 경우
			...
			store.doFileUpload(function() {
	        	store.load(baseUrl + "/DH0550/doSave", function() {
			...
			store.doFileUpload(function() {
	        	store.load(baseUrl + "/DH0550/doSave", function() {
		            ...
		            alert(this.getResponseMessage());
		            opener.doSearch();
		            location.href='/eversrm/master/bom/DH0550/view.so?SCREEN_ID=DH0550&popupFlag=true&itemList=false&EXEC_NUM='+this.getParameter("EXEC_NUM");
		        });
            });
		}
	}
	  	...
	  	store.doFileUpload(function() {
		   	store.load(baseUrl + "/doSave", function() {
		        ...
		        //alert(this.getParameter("EXEC_NUM"));
		            location.href='/eversrm/solicit/rfq/DH0550/view.so?SCREEN_ID=DH0550&popupFlag=true&EXEC_NUM='+this.getParameter("EXEC_NUM");
			    //if (popupFlag) {
				//	opener.doSearch();
			    //} else {
			    //}
    ...
    function doDelete() {
        ...
        store.setGrid([gridV]);
    	store.getGridData(gridV, 'all');
    	store.load(baseUrl + 'DH0550/doDelete', function(){
        	EVF.getComponent('TRANSACTION_FLAG').setValue('Y');
    ...
    function doSearchGwDocData() {
        ...
        store.setGrid([gwdoc]);
        store.load(baseUrl + "DH0550/doSearchGwDocData", function () {
        	gwdoc.checkAll(true);
    ...
    <e:window id="DH0550" onReady="init" initData="${initData}" title="${fullScreenName}">
