...
<e:ui locale="${ses.countryCd}">
  <script>
    ...
    var grid;
    var baseUrl = "/eversrm/master/srm/SRM_420";
    var selRow;
    ...
    // 승인 협력회사 가져오기
    function doApprovalVendor() {
      var store = new EVF.Store();
      store.setGrid([grid]);
      store.load(baseUrl + '/doApprovalVendor', function(){
        if(grid.getRowCount() == 0) {
          return alert('${msg.M0002}');
        }
      });
  ...
  <e:window id="SRM_420" onReady="init" initData="${initData}" title="${fullScreenName}" breadCrumbs="${breadCrumb }">
    ...
    <e:buttonBar width="100%" align="right">
      <e:button id="doApprovalVendor" name="doApprovalVendor" label="${doApprovalVendor_N}" onClick="doApprovalVendor" disabled="${doApprovalVendor_D}" visible="${doApprovalVendor_V}"/>
