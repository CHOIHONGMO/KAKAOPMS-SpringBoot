...
<script>
	...
	function onCellClick1(colId, rowId) {
	    ...
	    if (colId === "ITEM_CLS1" && gridClass.getCellValue(rowId, "INSERT_FLAG") !== "I") {
	        ...
	        store.load(baseUrl + "BBM_050/selectChildClass", function () {
	...
	function doSearch() {
        ...
        /*if(EVF.C("USE_FLAG").getValue()=="1" && EVF.C("ITEM_CLS_NM").getValue()==""){
                grid2.checkAll(true);
                grid2.delRow();
                grid3.checkAll(true);
                grid3.delRow();
                grid4.checkAll(true);
                grid4.delRow();
                EVF.C("ITEM_CLS_CLICKED").setValue(grid1.getCellValue(0, "ITEM_CLS1"));
                EVF.C("ITEM_CLS_TYPE_CLICKED").setValue(grid1.getCellValue(0, "ITEM_CLS_TYPE"));
            ...
            var store = new EVF.Store();
            store.load(baseUrl + "BBM_050/selectItemClass", function () {
	...
	function doSave() {
	    ...
	    if (getActiveRadio() !== "radio1" && "${keyRule}" === "auto") {
	        ...
	        for(var i in selectedRowIds) {
	            ...
	            if(EVF.isEmpty(colValue)) {
	                return alert('${BBM_050_0005}'); <%-- 코드명을 입력하시기 바랍니다. --%>
	            }
	        }
	    ...
	    if (getActiveRadio() == "radio1") {
	        ...
	        store.load(baseUrl + "BBM_050/saveItemClass", function () {
...
//	            onCellClick1("ITEM_CLS1", saveClassRow);
	...
	function doDelete() {
	    ...
	    store.setGrid([activeGrid]);
	    store.getGridData(activeGrid, 'sel');
	    store.load(baseUrl + "BBM_050/deleteItemClass", function () {
...
<e:window id="BBM_050" onReady="init" initData="${initData}" title="${fullScreenName}">
    <e:searchPanel id="form" title="${form_CAPTION_N}" labelWidth="${labelWidth}" labelAlign="${labelAlign}" columnCount="3" useTitleBar="false" onEnter="doSearch">
        <e:label for="ITEM_CLS" title="${form_ITEM_CLS_NM_N}"/>
        <e:field colSpan="3">
            <%--<e:select id="ITEM_CLS" name="ITEM_CLS" usePlaceHolder="" options="${refItemType}" required="${form_ITEM_CLS_R}" readOnly="${form_ITEM_CLS_RO}" disabled="${form_ITEM_CLS_D}" width="150"/>--%>
            <e:select id="ITEM_CLS" name="ITEM_CLS" value="${form.ITEM_CLS}" options="${itemClsOptions}" width="150" disabled="${form_ITEM_CLS_D}" readOnly="${form_ITEM_CLS_RO}" required="${form_ITEM_CLS_R}" placeHolder="" />

            <e:inputText id="ITEM_CLS_NM" name="ITEM_CLS_NM" width="${inputTextWidth }" maxLength="${form_ITEM_CLS_NM_M }" required="${form_ITEM_CLS_NM_R }" readOnly="${form_ITEM_CLS_NM_RO }" disabled="${form_ITEM_CLS_NM_D}" visible="${form_ITEM_CLS_NM_V}"/>
        </e:field>
        <e:label for="ITEM_CLS" title="${form_USE_FLAG_N}"/>
        <e:field>
            <e:select id="USE_FLAG" name="USE_FLAG" value="${form.USE_FLAG}" options="${useFlagOptions}" width="150" disabled="${form_USE_FLAG_D}" readOnly="${form_USE_FLAG_RO}" required="${form_USE_FLAG_R}" placeHolder="" />
