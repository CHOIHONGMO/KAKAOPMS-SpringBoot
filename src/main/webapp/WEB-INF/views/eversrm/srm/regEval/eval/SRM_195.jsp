...
<e:ui locale="${ses.countryCd}" lang="${ses.langCd}" dateFmt="${ses.dateFormat}">
	...
	function init() {
		...
		grid.cellClickEvent(function(rowid, celname, value, iRow, iCol) {
	        ...
	        if (celname == "EV_NUM") {
	            ...
	            everPopup.openPopupByScreenId('SRM_196', 950, 700, params);
	        }
		});
    ...
    function setUserA(dataJsonArray) {
        EVF.getComponent("EV_CTRL_USER_NM").setValue(dataJsonArray.USER_NM);
        EVF.getComponent("EV_CTRL_USER_ID").setValue(dataJsonArray.USER_ID);
    }

    function setUserB(dataJsonArray) {
        EVF.getComponent("EV_USER_NM").setValue(dataJsonArray.USER_NM);
        EVF.getComponent("EV_USER_ID").setValue(dataJsonArray.USER_ID);
    }
    ...
    function doSearch() {
        ...
        store.setGrid([grid]);
        store.load(baseUrl + 'SRM_195/doSearch', function() {
    ...
    function doComplete() {
        ...
        if (confirm("${msg.M8888}")) {
            ...
            store.load(baseUrl + 'SRM_195/doComplete', function() {
    ...
    <%-- 완료취소 --%>
     function doCancel() {
        ...
        if (confirm("${msg.M8888}")) {
            ...
            store.setGrid([grid]);
        	store.getGridData(grid, 'sel');
            store.load(baseUrl + 'SRM_195/doCancel', function() {
            	alert(this.getResponseMessage());
            	doSearch();
            });
    ...
    <e:window id="SRM_195" onReady="init" initData="${initData}" title="${fullScreenName}" breadCrumbs="${breadCrumb }">
        <e:searchPanel id="form" title="${msg.M9999 }" columnCount="3" labelWidth="${labelWidth }" useTitleBar="false" onEnter="doSearch">
        	<e:row>
			   ...
			   <%--진행상태--%>
				...
				<e:field>
				<e:search id="EV_CTRL_USER_NM" style="${imeMode}" name="EV_CTRL_USER_NM" value="" width="100%" maxLength="${form_EV_CTRL_USER_NM_M}" onIconClick="${form_EV_CTRL_USER_NM_RO ? 'everCommon.blank' : 'selectUserA'}" disabled="${form_EV_CTRL_USER_NM_D}" readOnly="${form_EV_CTL_USER_NM_RO}" required="${form_EV_CTRL_USER_NM_R}" />
		...
		<e:buttonBar id="buttonBar" align="right" width="100%">
			...
			<e:button id="doRegEval" name="doRegEval" label="${doRegEval_N}" onClick="doRegEval" disabled="${doRegEval_D}" visible="${doRegEval_V}"/>
